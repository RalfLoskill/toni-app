...
# TONI Auth V7.6 – Fix Passwort-Dialog

Diese Version behebt, dass der Dialog „Passwort festlegen“ nach dem Passwort-Reset offen bleibt.
...
# TONI Auth V7.7 – Fix nach Verifizierungslink

Diese Version behebt den Fall, dass nach Klick auf die Verifizierungs-Mail nur das Dashboard erscheint.

Einrichtung:
1. In Supabase `sql/auth_registration_after_verify_v7_7.sql` ausführen.
2. `public/index.html` ersetzen.
3. Neu veröffentlichen.
...
- Admin-Login wird erkannt, aber Dashboard wechselt nicht in Admin-Modus.
- Nach Verifizierungslink öffnet sich die Profilvervollständigung nicht zuverlässig.
- Profilabschluss schließt Modal und aktualisiert Dashboard.

Einrichtung:

1. In Supabase `sql/auth_stabilized_v8.sql` ausführen.
2. `public/index.html` ersetzen.
3. Neu veröffentlichen.
4. Browser hart neu laden.
5. Für Admin prüfen:
   select email, role, profile_complete from profiles where email='ralf.loskill@googlemail.com';


# TONI Auth V9 – robuster Login- und Registrierungsflow

Diese Version prüft den Anmeldeprozess erneut vollständig.

Behoben:
- Login bleibt bei „Anmeldung läuft …“ hängen.
- Admin-Login aktualisiert das Dashboard nicht.
- Nach Verifizierungslink öffnet sich „Registrierung abschließen“ nicht.
- Profilabschluss schließt Modal nicht zuverlässig.

Einrichtung:
1. In Supabase `sql/auth_robust_v9.sql` ausführen.
2. `public/index.html` ersetzen.
3. Neu veröffentlichen.
4. Browser hart neu laden / localStorage.clear().


# TONI Auth V10 – Fokus Admin-Anmeldung

Diese Version konzentriert sich nur auf den Admin-Anmeldeprozess.

Änderungen:
- Admin-Login nutzt Supabase Auth REST API direkt.
- Hängende Supabase-JS Login-Aufrufe werden umgangen.
- Nach erfolgreichem Login wird das Dashboard hart in den Admin-Modus gesetzt.
- Passwort-Reset wird ebenfalls robuster verarbeitet.

Einrichtung:
1. In Supabase `sql/auth_admin_focus_v10.sql` ausführen.
2. `public/index.html` ersetzen.
3. Neu veröffentlichen.
4. Browser hart neu laden und `localStorage.clear()` ausführen.
