...
async function processJob(job, ANTHROPIC_API_KEY) {
  try {
    let res;
    if (job.agent_key === 'find_leads') res = await runFindLeads(job, ANTHROPIC_API_KEY);
    else if (job.agent_key === 'lead_qualify') res = await runLeadQualify(job, ANTHROPIC_API_KEY);
    else throw new Error(`unbekannter agent_key: ${job.agent_key}`);
