...
Bilder: In dieser Version KEINE Bilder/SVGs einbetten. Lass blocks und cover_image leer.
Inhalt: didaktisch sinnvoll, fachlich korrekt, Deutsch, altersgerecht. Pro Station mehrere Aufgaben; über die Reise alle fünf Typen.`;
    ...
    try {
      ...
      if (!journey) {
        console.error('journey_builder JSON-Parse-Fehler. stop_reason=' + stopReason + ' len=' + text.length + ' err=' + (parseErr && parseErr.message));
        return res.status(422).json({
          error: 'Die KI-Antwort war kein gültiges JSON.',
          diagnose: {
            stop_reason: stopReason,
            antwort_laenge: text.length,
            parse_fehler: parseErr && parseErr.message,
