...
Inhalt: didaktisch sinnvoll, fachlich korrekt, Deutsch, altersgerecht. Pro Station mehrere Aufgaben; über die Reise alle fünf Typen.`;
    ...
    try {
      ...
      try {
        ...
        // DIAGNOSE: stop_reason verrät, ob die Antwort abgeschnitten wurde
        // ("max_tokens") oder vollständig war ("end_turn"). Roh-Ausschnitte
        // helfen zu sehen, was tatsächlich kam.
        console.error('journey_builder JSON-Parse-Fehler. stop_reason=' + stopReason + ' len=' + text.length);
        return res.status(422).json({
          error: 'Die KI-Antwort war kein gültiges JSON.',
          diagnose: {
            stop_reason: stopReason,
            antwort_laenge: text.length,
