...
begin
  ...
  insert into public.profiles (id,display_name,email,class_name,role,password_set,profile_complete,force_password_change)
  values (new.id,coalesce(new.raw_user_meta_data->>'display_name',split_part(new.email,'@',1)),new.email,'',v_role,false,false,true)
  on conflict (id) do update
  set email=excluded.email,
      role=case when exists (select 1 from public.admin_emails where lower(email)=lower(new.email)) then 'admin' else public.profiles.role end,
      updated_at=now();
  return new;
end;
$$;
...
begin
  select exists (
    ...
    where lower(email)=lower(trim(p_email)) and is_active=true and profile_complete=true
  ) into v_exists;
...
create or replace function public.complete_my_profile(p_first_name text,p_last_name text,p_class_name text)
returns jsonb
language plpgsql
security definer
set search_path = public
...
begin
  ...
  if length(v_display_name) < 2 then raise exception 'name_required'; end if;
  ...
  return jsonb_build_object('ok',true,'profile_id',auth.uid(),'display_name',v_display_name);
end;
$$;
...
begin
  if auth.uid() is null then raise exception 'not_authenticated'; end if;
  update public.profiles set password_set=true, force_password_change=false, updated_at=now() where id=auth.uid();
