...
(function(){
  ...
  function resolveInviteId(){
    ...
    // Fremder Magic-Link-Callback (Student/Tutor) ohne admininvite? Dann nicht
    // über alten sessionStorage-Rest anspringen (sonst Session-Kaperung).
    try{
      const u=new URL(window.location.href);
      const h=new URLSearchParams((window.location.hash||"").replace(/^#/,""));
      const istCallback = u.searchParams.has("code") || u.searchParams.has("token_hash") ||
                          h.has("access_token") || h.has("refresh_token") || h.has("error");
      if(istCallback && !u.searchParams.has("admininvite")) return null;
    }catch(e){}
...
(function(){
  ...
  function resolveTutorInviteId(){
    ...
    if(fromUrl) return fromUrl;
    // Wenn die URL Magic-Link-Callback-Parameter trägt (code/token_hash/Hash-Token),
    // aber KEINEN tutorinvite-Parameter, gehört dieser Magic-Link zu einem ANDEREN
    // Flow (z.B. Student/Admin). Dann darf der Tutor-Flow NICHT über einen alten
    // sessionStorage-Rest anspringen – sonst kapert er die Session/URL und der
    // eigentliche Callback scheitert ("keine Sitzung erstellt").
    try{
      const u=new URL(window.location.href);
      const h=new URLSearchParams((window.location.hash||"").replace(/^#/,""));
      const istCallback = u.searchParams.has("code") || u.searchParams.has("token_hash") ||
                          h.has("access_token") || h.has("refresh_token") || h.has("error");
      if(istCallback && !u.searchParams.has("tutorinvite")) return null;
      if(u.searchParams.has("admininvite")) return null; // fremder Einladungstyp
    }catch(e){}
