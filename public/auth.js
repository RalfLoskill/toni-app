...
function toniApplyRegistrationVariant(){
  ...
  if(isAdmin){
    if(title) title.textContent = "Registrierung als Admin abschließen.";
    ...
    if(classRow) classRow.style.display = "none"; // Klasse/Kurs entfällt für Admins
...
window.completeSelfRegistration = async function() {
  ...
  // Studenten (für Admins ausgeblendet und nicht erforderlich).
  if (!first || !last) {
    setRegistrationMessage(isAdminReg ? "Bitte füllen Sie alle Pflichtfelder aus (Vorname und Nachname)." : "Bitte gib Vorname und Nachname ein.", "err");
    return;
  }
  ...
  try {
    ...
    setTimeout(async () => {
      closeRegistrationRequiredModal?.();
      if(isAdminReg){
        // Admin: frische Neuanmeldung erzwingen (wie beim Studenten-Flow).
        try{ sessionStorage.removeItem("toni_admin_registration"); }catch(e){}
        try{ window.TONI_PENDING_ADMIN_REGISTRATION = false; }catch(e){}
        try{
          const client = getSupabaseClient();
          await Promise.race([client.auth.signOut(), new Promise(r=>setTimeout(r,4000))]);
        }catch(e){ console.warn("Abmeldung nach Admin-Registrierung:", e); }
        // URL säubern und Login-Fenster öffnen.
