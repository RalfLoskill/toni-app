...
(function(){
  ...
  window.toniV120CreateInstitution = async function(){
    ...
    try{
      ...
      // Direkt eine Admin-Einladung für die neue Institution erzeugen
      let inviteHtml = "";
      try{
        const inv = await callRpc("rpc/superadmin_create_admin_invite_v121", {
          p_username: SUPERADMIN_USERNAME,
          p_password: SUPERADMIN_PASSWORD,
          p_institution_id: result.institution_id
        });
        if(inv?.ok && inv.invite_id){
          const link = toniBuildAdminInviteLink(inv.invite_id);
          inviteHtml = `<div style="margin-top:10px;padding:10px 12px;background:var(--color-background-secondary);border-radius:var(--border-radius-md);font-size:13px">
            <div style="margin-bottom:6px;color:var(--color-text-secondary)">Einladungslink für den Admin (gültig 30 Tage):</div>
            <div style="display:flex;gap:8px;align-items:center">
              <input type="text" readonly value="${esc(link)}" onclick="this.select()" style="flex:1;padding:6px 8px;border:0.5px solid var(--color-border-secondary);border-radius:6px;font-size:12px;font-family:ui-monospace,Menlo,Consolas,monospace;background:#fff;color:var(--color-text-primary)">
              <button onclick="toniV121CopyText(event,'${esc(link)}')" style="padding:6px 12px;background:#185FA5;color:#fff;border:none;border-radius:6px;font-size:12px;cursor:pointer;white-space:nowrap">Kopieren</button>
            </div>
          </div>`;
        }
      }catch(invErr){ console.warn("Admin-Einladung beim Anlegen:", invErr); }

      setMsg(`✅ Institution „${esc(result.institution_name)}" angelegt.<br>Kennung: <strong>${esc(result.internal_code)}</strong>${inviteHtml}`, "ok");
      // Übersicht aktualisieren
      if(typeof window.toniV118LoadInstitutionStats === "function") window.toniV118LoadInstitutionStats();
      // Fenster NICHT automatisch schließen, damit der Link kopiert werden kann.
    }catch(e){
