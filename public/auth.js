  ...
  async function tryAcceptTutorAfterLogin(inviteId){
    ...
    try{
      ...
      if(res?.ok){
        ...
        console.warn("Tutor-Einladung einlösen:", res?.error);
        acceptDone=false;
      }
    }catch(e){ console.warn("accept_tutor_self_invite:", e); acceptDone=false; }
  }

  window.addEventListener("DOMContentLoaded", ()=>{
    const inviteId=resolveTutorInviteId();
    if(!inviteId) return;
    // Falls die URL den Parameter trägt (vor Magic-Link), Einlöse-Seite zeigen.
    if(getTutorInviteIdFromUrl()){ setTimeout(()=>openTutorAcceptFlow(inviteId),400); }
    // Nach Magic-Link mehrfach einlösen versuchen.
