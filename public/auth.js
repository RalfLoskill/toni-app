...
/* ============================================================
   TONI – SuperAdmin Institutionen-Übersicht (v118)
   Eigenes Modul, gleiches Sicherheitsmuster wie die Admin-Liste:
   p_username/p_password werden serverseitig in der RPC geprüft.
   ============================================================ */
(function(){
  const SUPERADMIN_USERNAME = "SuperAdmin";
  const SUPERADMIN_PASSWORD = window.SUPERADMIN_PASSWORD || "SuperAdmin#";
  ...
  // Bytes kompakt formatieren (B / KB / MB / GB).
  ...
  function renderInstitutionStats(list){
    ...
    rows.forEach(r=>{
      ...
      sumTutor   += Number(r.tutor_count)||0;
      sumStudent += Number(r.student_count)||0;
      sumImg     += Number(r.image_count)||0;
      sumDoc     += Number(r.doc_count)||0;
      sumBytes   += Number(r.total_bytes)||0;
    });
    const sumAccounts = sumAdmin + sumTutor + sumStudent;
    const sumFiles = sumImg + sumDoc;

    const cards = `
      <div class="sa-inst-cards">
        <div class="sa-inst-card"><div class="sa-inst-card-label">Institutionen</div><div class="sa-inst-card-value">${rows.length}</div></div>
        <div class="sa-inst-card"><div class="sa-inst-card-label">Konten gesamt</div><div class="sa-inst-card-value">${sumAccounts}</div></div>
        <div class="sa-inst-card"><div class="sa-inst-card-label">Dateien</div><div class="sa-inst-card-value">${sumFiles}</div></div>
  ...
  async function loadInstitutionStats(event){
    if(event){ event.preventDefault?.(); event.stopPropagation?.(); }
    const root = document.getElementById("superadmin-institutions-content");
    if(root) root.innerHTML = `<div class="assignment-empty">Übersicht wird geladen …</div>`;
    try{
      const result = await callRpc("rpc/superadmin_institution_stats_v118", authValue());
      const list = Array.isArray(result) ? result : (result?.institutions || []);
      renderInstitutionStats(list);
    }catch(e){
      console.warn("Institutionen-Übersicht:", e);
      if(root) root.innerHTML = `<div class="assignment-empty">Übersicht konnte nicht geladen werden.</div>`;
    }
  }

  function toggleInstitutionsPanel(){
    const body = document.getElementById("superadmin-institutions-body");
    const note = document.getElementById("superadmin-institutions-collapsed-note");
    const symbol = document.getElementById("superadmin-institutions-toggle-symbol");
    if(!body) return;
...
(function(){
  ...
  function resolveTutorInviteId(){
    ...
    if(fromUrl) return fromUrl;
    // Fremder Einladungstyp in URL? Dann Tutor-Flow nicht über sessionStorage starten.
    try{ if(new URL(window.location.href).searchParams.has("admininvite")) return null; }catch(e){}
    const p=readPendingTutorInvite();
    return p?p.inviteId:null;
  }

  async function tryAcceptTutorAfterLogin(inviteId){
    if(acceptDone||!inviteId) return;
    const client=(typeof getSupabaseClient==="function")?getSupabaseClient():null;
    if(!client) return;
    let session=null;
    try{ const { data }=await client.auth.getSession(); session=data?.session; }catch(e){}
    if(!session?.user) return;
    acceptDone=true;
    try{ console.log("[TUTOR-ONBOARDING] rufe accept-RPC, inviteId=" + inviteId); }catch(e){}
    try{
      const token=session.access_token;
  ...
  window.addEventListener("DOMContentLoaded", ()=>{
    // Wenn ein Tutor-Link aktiv ist, einen evtl. alten Admin-Einladungs-Rest
    // entfernen (verhindert, dass der Admin-Flow fälschlich anspringt).
    try{
    ...
    // Diagnose-Helfer: zeigt im Fenstertitel/Konsole, welcher Schritt läuft.
    function diag(stufe){ try{ console.log("[TUTOR-ONBOARDING]", stufe); window.__TONI_TUTOR_DIAG = (window.__TONI_TUTOR_DIAG||[]); window.__TONI_TUTOR_DIAG.push(stufe); }catch(e){} }
    diag("DOMContentLoaded, inviteId=" + inviteId + ", urlParam=" + !!getTutorInviteIdFromUrl());

    // Erkennen, ob dies ein Magic-Link-Rücksprung ist (Token in Query/Hash).
    // Falls NICHT -> erster Aufruf -> E-Mail-Seite SOFORT zeigen (kein Warten).
    function istMagicLinkRueckkehr(){
      try{
        const u=new URL(window.location.href);
        const h=new URLSearchParams((window.location.hash||"").replace(/^#/,""));
        return u.searchParams.has("code") || u.searchParams.has("token_hash") ||
               h.has("access_token") || h.has("refresh_token") || h.has("error");
      }catch(e){ return false; }
    }

    if(!istMagicLinkRueckkehr() && getTutorInviteIdFromUrl()){
      // Möglicher erster Aufruf (kein Token im Hash). ABER: Supabase könnte den
      // Token bereits verarbeitet haben (Session vorhanden, Hash leer). Daher
      // ZUERST kurz die Session prüfen; nur wenn keine Session -> E-Mail-Seite.
      (async ()=>{
        const client=(typeof getSupabaseClient==="function")?getSupabaseClient():null;
        let session=null;
        try{ const { data }=await (client?.auth?.getSession?.() || Promise.resolve({})); session=data?.session||null; }catch(e){}
        if(session?.user){
          diag("kein Token, aber Session vorhanden -> einlösen");
          tryAcceptTutorAfterLogin(inviteId);
        }else{
          diag("erster Aufruf (kein Token, keine Session) -> E-Mail-Seite sofort");
          openTutorAcceptFlow(inviteId);
        }
      })();
    }else{
      // Magic-Link-Rücksprung ODER Param nur in sessionStorage -> auf Session warten und einlösen.
      (async ()=>{
        const client=(typeof getSupabaseClient==="function")?getSupabaseClient():null;
        ...
        diag("Session nach Warten: " + (session?.user ? ("JA ("+session.user.email+")") : "NEIN"));
        if(session?.user){
          tryAcceptTutorAfterLogin(inviteId);
        }else if(getTutorInviteIdFromUrl()){
          diag("zeige E-Mail-Seite (keine Session nach Rücksprung)");
          openTutorAcceptFlow(inviteId);
        }
      })();
    }

    // Zusätzliche Einlöse-Versuche, falls die Session erst später entsteht.
    [600,1400,2600,4000,6000,9000].forEach(ms=>setTimeout(()=>tryAcceptTutorAfterLogin(inviteId),ms));
    try{
      const client=(typeof getSupabaseClient==="function")?getSupabaseClient():null;
      client?.auth?.onAuthStateChange((event)=>{
        diag("onAuthStateChange: " + event);
        if(event==="SIGNED_IN"||event==="INITIAL_SESSION"){ setTimeout(()=>tryAcceptTutorAfterLogin(inviteId),200); }
      });
    }catch(e){}
    // Login-Modal-Wächter, solange Einlöse-Seite offen ist.
    const guard=setInterval(()=>{
      const mine=document.getElementById("tutor-invite-accept-modal");
      if(mine && mine.classList.contains("open")){ document.getElementById("auth-modal")?.classList.remove("open"); }
    },300);
    setTimeout(()=>clearInterval(guard),30000);
  });
