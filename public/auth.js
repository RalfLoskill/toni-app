...
(function(){
  ...
  async function sendInviteMagicLink(inviteId, institutionId){
    ...
    try{
      ...
      if(error) throw error;
      // E-Mail lokal vormerken, damit Etappe 4 sie kennt (Verknüpfung).
      try{ sessionStorage.setItem("toni_admin_invite_pending", JSON.stringify({ inviteId, institutionId, email })); }catch(e){}
      setMsg("✅ Anmeldelink gesendet. Bitte öffne die E-Mail und klicke den Link. Danach schließt du deine Registrierung ab.");
    }catch(e){
  ...
  // Wenn der Nutzer (zurück auf ?admininvite=) eine Session hat, das Profil
  // zum Admin der Institution machen – BEVOR der Registrierungs-Flow greift.
  ...
  window.addEventListener("DOMContentLoaded", ()=>{
    ...
    const tries = [800, 1600, 2800, 4200, 6000];
