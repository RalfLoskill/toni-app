...
async function toniRequestSocraticHint() {
  ...
  // Journey-Kontext enthält openTask inkl. studentAnswer
  const jc = (typeof toniBuildJourneyContext === 'function') ? toniBuildJourneyContext() : null;
  const openTask = jc && jc.openTask ? jc.openTask : null;

  const stufenText = {
    1: 'Stufe 1 von 3: Gib NUR eine offene Leitfrage oder einen Denkanstoß. Verrate nichts Konkretes.',
    2: 'Stufe 2 von 3: Werde etwas konkreter – nenne den nächsten Denkschritt oder die relevante Idee, aber NICHT die Lösung.',
    3: 'Stufe 3 von 3: Gib einen sehr konkreten Tipp, der fast zur Lösung führt – aber nenne die finale Antwort NICHT.'
  }[level];
  ...
  const hintContext = {
    ...
    socraticHint: {
      regeln: [
        'Du bist TONi, ein geduldiger Lernbegleiter für Schüler.',
        'Verrate NIEMALS die fertige Lösung oder das Endergebnis.',
        'Hilf dem Schüler, selbst auf die Lösung zu kommen.',
        'Sei ermutigend, kurz (2–4 Sätze) und altersgerecht.',
        'Wenn der Schüler schon etwas geschrieben hat, gehe konkret darauf ein: was ist gut, was fehlt noch?',
        stufenText
      ].join(' '),
      stufe: level,
      aufgabe: openTask ? {
        titel: openTask.title, typ: openTask.type, station: openTask.station,
        beschreibung: openTask.prompt
      } : null,
      bisherigeAntwort: openTask ? (openTask.studentAnswer || '') : ''
    }
  };
  ...
  try {
    const result = await callAgent('explanation_agent', hintContext);
