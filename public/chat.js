      ...
      if (isOpen && selId && typeof window.activeJourney === 'function') {
        ...
        if (found) {
          ...
          if (t.type && String(t.type).toLowerCase().includes('quiz') &&
            ...
            const qs = t.quiz_data.questions.map((q, i) => {
              const opts = Array.isArray(q.options)
                ? ' Antwortmöglichkeiten: ' + q.options.map((o, oi) => `${String.fromCharCode(65+oi)}) ${o}`).join('; ')
                : '';
              return `Frage ${i+1}: ${q.question || ''}${opts}`;
            }).join('\n');
            promptText = (promptText ? promptText + '\n\n' : '') +
              'Dies ist ein Quiz mit folgenden Fragen:\n' + qs +
              '\n(Hilf dem Schüler beim Nachdenken über die Fragen, nenne aber NICHT die richtige Antwort.)';
          }
          openTask = {
            title: t.title || '',
            type: t.type || '',
