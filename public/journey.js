...
async function startOnboardingVerification(){
  ...
  try{
    if(typeof sendVerificationMail==="function"){
      ...
      setOnboardingMessage("⚠️ Der Versand ist gerade nicht möglich. Bitte versuche es später erneut.","err");
    }
  }catch(e){
    console.error(e);
    setOnboardingMessage("⚠️ Der Link konnte nicht gesendet werden:<br>"+escapeHtml(e.message),"err");
  }finally{
...
function toniMaybeOpenOnboarding(reason){
  try{
    ...
    const hatCode = (typeof getJoinCodeFromUrl==="function" && getJoinCodeFromUrl())
                 || (typeof getJourneyCodeFromUrl==="function" && getJourneyCodeFromUrl());
    if(!hatCode){ document.documentElement.classList.remove("toni-onboarding-pending"); return false; }
    const eingeloggt = (typeof TONI_AUTH_SESSION!=="undefined" && TONI_AUTH_SESSION?.user)
                 || (window.TONI_AUTH_SESSION?.user);
    if(eingeloggt){
      // Eingeloggt → kein Onboarding-Modal; Code löst handleJoinLinkAfterLogin direkt ein.
      // Guard aufheben, damit die normale Oberfläche sichtbar ist.
      document.documentElement.classList.remove("toni-onboarding-pending");
      if(typeof handleJoinLinkAfterLogin==="function") handleJoinLinkAfterLogin();
      return false;
    }
