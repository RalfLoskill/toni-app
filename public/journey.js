...
async function loadAdminLearningJourneys(){
  ...
  const p = (async () => {
    try{
      ...
      // Zentral: Bei Studenten immer auch den sichtbaren "Eigene Lernreisen"-
      // Abschnitt aktualisieren. Damit sind ALLE Aufrufstellen (Import, Speichern,
      // Löschen, Duplizieren) automatisch abgedeckt – der Panel-Container ist beim
      // Studenten versteckt, sein eigener Container muss aber aktuell bleiben.
      try{
        const role = (window.TONI_AUTH_PROFILE && TONI_AUTH_PROFILE.role) || localStorage.getItem("toni_role") || "student";
        if(role === "student"
           && typeof toniLoadStudentOwnJourneys === "function"
           && document.getElementById("student-admin-journey-list")){
...
async function toniV50RenderAllJourneysInActivities(){
  ...
  try{
    ...
    // NUR für Studenten: eigenen Verwaltungsabschnitt UNTER den zugeordneten
    // Lernreisen anzeigen (eigene Lernreisen anlegen/bearbeiten im Tutor-Design).
    // Der obige Block (zugeordnete Lernreisen) bleibt davon unberührt.
    try{
      const role = (window.TONI_AUTH_PROFILE && TONI_AUTH_PROFILE.role) || localStorage.getItem("toni_role") || "student";
      if(role === "student" && typeof toniRenderStudentJourneyManager === "function"){
        toniRenderStudentJourneyManager(container);
      }
    }catch(e){ console.warn("Studenten-Lernreise-Verwaltung:", e); }
  }finally{
    window.TONI_V50_ACTIVITY_RENDERING = false;
  }
}
