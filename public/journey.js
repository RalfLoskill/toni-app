...
function toniInjectTimelineStyles(){
  ...
  css.textContent=`
    ...
    .toni-tl-dot.current{background:#185FA5;color:#fff;width:40px;height:40px;box-shadow:0 0 0 4px #E6F1FB}
    ...
    .toni-slim-task{display:flex;align-items:center;gap:8px;padding:9px 11px;border-radius:8px;font-size:13px;background:var(--color-background-secondary,#f7f6f2);border:0.5px solid transparent;margin-bottom:6px;transition:background .15s,border-color .15s}
    .toni-slim-task[onclick]{cursor:pointer}
    .toni-slim-task[onclick]:hover{background:var(--color-background-primary,#fff);border-color:var(--color-border-secondary,#ccc)}
    .toni-slim-task.current{border-color:#185FA5;background:var(--color-background-primary,#fff)}
...
(function(){
  ...
  function toniSlimTaskRow(task, opts){
    ...
    const icon = (typeof toniTypeIcon==="function" && typeof toniNormalizeType==="function")
      ? toniTypeIcon(toniNormalizeType(task.type)) : "📌";
    let statusIcon, metaHtml, cls="";
    if(task.status==="done"||task.done||task.completed){
      statusIcon='<span style="color:#639922">✅</span>';
      metaHtml='<span class="toni-slim-meta">erledigt</span>';
    }else if(!clickable){
      statusIcon='<span>🔒</span>';
      metaHtml='<span class="toni-slim-meta">gesperrt</span>';
      cls="locked";
    }else if(opts.current){
      statusIcon=`<span>${icon}</span>`;
      metaHtml='<span class="toni-slim-meta cur">jetzt dran</span>';
      cls="current";
    }else{
      statusIcon=`<span>${icon}</span>`;
      metaHtml=`<span class="toni-slim-meta">${task.required?'Pflicht':'optional'}</span>`;
    }
    const tname=`<span class="toni-slim-name ${(task.status==='done'||task.done)?'done':''}">${esc(task.title||'Aufgabe')}</span>`;
    if(clickable){
      return `<div class="toni-slim-task ${cls}" onclick="openLearningTask('${esc(task.id)}')">${statusIcon}${tname}${metaHtml}<span class="toni-slim-arr">→</span></div>`;
    }
    return `<div class="toni-slim-task ${cls}">${statusIcon}${tname}${metaHtml}</div>`;
  }
