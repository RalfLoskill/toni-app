...
function renderStudentList(){
  ...
  root.innerHTML=`<table class="student-table">
    <thead><tr><th>Name</th><th>Klasse</th><th>Lerngruppe(n)</th><th>E-Mail</th><th>Status</th></tr></thead>
    <tbody>${rows}</tbody>
  </table>`;
}

// Baustein 3: Einladungs-Modal für reine Registrierung (ohne Lerngruppe).
// Der Link trägt nur ?register=1 – KEINEN Gruppen-/Lernreise-Code.
function buildRegisterLink(){
  return `${window.location.origin}${window.location.pathname}?register=1`;
}
function toniOpenStudentInvite(){
  if(!canManageGroups()) return;
  ...
  const msg=document.getElementById("register-invite-message"); if(msg){ msg.className="auth-message"; msg.innerHTML=""; }
...
function downloadRegisterQrCode(){
  const link=buildRegisterLink();
