...
const DEFAULT_LEARNING_JOURNEYS = [{
  ...
  steps:[
    ...
    {id:'ohm',title:'Ohmsches Gesetz',subtitle:'Rechnen & anwenden',description:'Du nutzt U = R · I und stellst die Formel sicher um.',tasks:[
      {id:'ohm-1',title:'Formeln verstehen',type:'Lerninhalt',status:'done',required:true,description:'Erkläre U = R · I.',content:'Beschreibe mit eigenen Worten, wofür U, R und I stehen.'},
...
// Gemeinsame Medien-Anzeige (Bild / YouTube) für Aufgabentypen
function toniRenderTaskMedia(task){
  const blocks=Array.isArray(task.blocks)?task.blocks:[];
  const parts=[];
  const ytId=String(task.youtube_video_id||task.video_id||"");
  if(/^[A-Za-z0-9_-]{11}$/.test(ytId)){
    parts.push(`<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:10px;background:#000;margin-bottom:10px"><iframe src="https://www.youtube.com/embed/${ytId}" frameborder="0" allowfullscreen style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div>`);
  }
  blocks.forEach(b=>{
...
function toniRenderQuiz(task,el){
  ...
  if(!qdata?.questions?.length){
    el.innerHTML=`${media}<div style="font-size:14px;color:var(--color-text-primary)">${toniEsc(task.content||task.description||'')}</div><div style="margin-top:10px;background:#FAEEDA;border-radius:8px;padding:8px 12px;font-size:12px;color:#633806">🎯 Quiz-Fragen werden noch vom Tutor erstellt.</div>`;
    return;
  }
  const idx=task._quizIndex||0;
  const q=qdata.questions[idx];
  el.innerHTML=`${media}<div style="font-size:12px;color:var(--color-text-tertiary);margin-bottom:8px">Frage ${idx+1} von ${qdata.questions.length}</div><div style="font-size:14px;font-weight:500;color:var(--color-text-primary);margin-bottom:12px">${toniEsc(q.question)}</div><div id="quiz-options">${q.options.map((opt,i)=>`<button onclick="toniQuizAnswer(${i})" style="display:block;width:100%;text-align:left;padding:10px 14px;margin-bottom:6px;background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;font-size:13px;cursor:pointer;color:var(--color-text-primary)">${String.fromCharCode(65+i)}. ${toniEsc(opt)}</button>`).join('')}</div>`;
}
window.toniQuizAnswer=function(idx){
  const task=findTask(STATE.selectedTaskId)?.task;
  if(!task?.quiz_data) return;
  const curIdx=task._quizIndex||0;
  const q=task.quiz_data.questions[curIdx];
  ...
  // Weiter-Button oder Auswertung
  const total=task.quiz_data.questions.length;
  const nav=document.createElement('div');
  nav.style.cssText='margin-top:12px';
  if(curIdx<total-1){
    nav.innerHTML=`<button onclick="toniQuizNext()" style="padding:10px 18px;background:#185FA5;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer">Nächste Frage →</button>`;
  }else{
    nav.innerHTML=`<div style="background:var(--color-background-secondary);border-radius:8px;padding:12px;font-size:14px;color:var(--color-text-primary)">🎉 Quiz abgeschlossen: <strong>${task._quizCorrect||0} von ${total}</strong> richtig.</div>`;
  }
  document.getElementById('quiz-options').appendChild(nav);
};
window.toniQuizNext=function(){
  const f=findTask(STATE.selectedTaskId);
  if(!f?.task?.quiz_data) return;
  f.task._quizIndex=(f.task._quizIndex||0)+1;
  const contentEl=document.getElementById('lr-task-content');
  if(contentEl) toniRenderQuiz(f.task, contentEl);
};

function toniRenderReflexion(task,el){
  if(!el) return;
  const prompt = task.reflexion_prompt||task.content||'Was hast du heute gelernt? Was war schwierig?';
  // Mehrere Selbsteinschätzungen: neues Format reflexion_scales[], altes reflexion_scale/-label als Fallback
  let scales = Array.isArray(task.reflexion_scales) ? task.reflexion_scales.slice() : [];
  if(!scales.length && task.reflexion_scale !== false){
    scales = [{label: task.reflexion_scale_label || 'Wie gut verstehst du das Thema?'}];
  }
  const helpers = Array.isArray(task.reflexion_helpers) ? task.reflexion_helpers.filter(h=>String(h||'').trim()) : [];
  const blocks = Array.isArray(task.blocks) ? task.blocks : [];
  const minLen = parseInt(task.reflexion_min_length,10) || 0;

  // Material (Bild / YouTube-Video) anzeigen
  const mediaHtml = blocks.map(b=>{
    if(b.type==='image' && b.url) return `<img src="${toniEsc(b.url)}" alt="${toniEsc(b.alt||'')}" style="max-width:100%;border-radius:10px;margin-bottom:10px">`;
    if(b.type==='link' && b.youtube && b.url){
      const m=String(b.url).match(/[A-Za-z0-9_-]{11}/);
      if(m) return `<div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:10px;background:#000;margin-bottom:10px"><iframe src="https://www.youtube.com/embed/${m[0]}" frameborder="0" allowfullscreen style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe></div>`;
    }
    return '';
  }).join('');
...
function completeSelectedLearningTask(){
  ...
  if(f && toniNormalizeType(f.task.type)==='Reflexion'){
    const minLen=parseInt(f.task.reflexion_min_length,10)||0;
    if(minLen>0 && f.task.required){
      const answer=(document.getElementById('lr-answer')?.value||'').trim();
      if(answer.length<minLen){
        alert(`Bitte schreibe mindestens ${minLen} Zeichen, bevor du fortfährst (aktuell ${answer.length}).`);
        return;
      }
    }
  }
...
async function loadInstitutionStudents(){
  ...
  try{
    ...
    // Datei-Statistik pro Student in EINEM Aufruf nachziehen – nur für Admin/SuperAdmin
    // (Tutoren sehen die Speicher-Spalten nicht, sparen wir uns die Abfrage).
    const istAdminFS = (typeof getCurrentRole==="function" ? getCurrentRole() : localStorage.getItem("toni_role")) === "admin"
...
function renderStudentList(){
  ...
  // Spalten rollenabhängig:
  //  - Tutor sieht die Lerngruppen-Spalte (wie bisher), KEINE Speicher-Spalten.
  //  - Admin/SuperAdmin sieht die Speicher-/Datei-Spalten, KEINE Lerngruppen-Spalte.
  const zeigeGruppen  = !istAdmin;
  const zeigeSpeicher = istAdmin;
  if(!list.length){
  ...
  const rows=pageItems.map(s=>{
    ...
    const del = darfLoeschen
      ...
      : "";
    // Datei-Statistik (Bilder / Dokumente / Speicher mit Balken) – nur für Admin
    ...
    const storageCell = fs.bytes > 0
      ...
      : `<span class="student-muted">–</span>`;
    const speicherZellen = zeigeSpeicher
  ...
  // Institutionsweite Summen für die Übersichtszeile (nur Admin).
  let summary = "";
  if(zeigeSpeicher){
    ...
    summary = `<div class="student-storage-summary">
      <span>Gesamt:</span>
  ...
  root.innerHTML=`${summary}<table class="student-table">
    <thead><tr><th>${nameHead}</th><th>${klasseHead}</th>${zeigeGruppen?"<th>Lerngruppe(n)</th>":""}${zeigeSpeicher?`<th style="text-align:center" title="Anzahl Bilder">🖼️</th><th style="text-align:center" title="Anzahl Dokumente">📄</th><th>Speicher</th>`:""}<th>E-Mail</th><th>${statusHead}</th>${darfLoeschen?"<th></th>":""}</tr></thead>
    <tbody>${rows}</tbody>
  </table>${pager}`;
}
...
// Bezeichnung einer Lerngruppe ändern (Prompt + UPDATE).
async function renameLearningGroup(id){
  const g=(TONI_GROUPS||[]).find(x=>String(x.id)===String(id)); if(!g)return;
  const neu=(window.prompt("Neue Bezeichnung für die Lerngruppe:", g.name)||"").trim();
  if(!neu||neu===g.name)return;
  try{
    await supabaseRequest(`learning_groups?id=eq.${encodeURIComponent(id)}`,{
      method:"PATCH",
      headers:{"Prefer":"return=minimal"},
      body:JSON.stringify({name:neu})
    });
    await loadTutorGroups();
    if(String(ACTIVE_GROUP_ID)===String(id)){
      const t=document.getElementById("active-group-title");
      if(t)t.textContent="Mitglieder der Lerngruppe "+neu;
    }
  }catch(e){
    console.warn("Lerngruppe umbenennen fehlgeschlagen:",e);
    alert("Die Bezeichnung konnte nicht geändert werden:\n"+(e.message||e));
  }
}

// Lerngruppe löschen. Entfernt NUR die Gruppe und ihre Mitgliedschaften
// (learning_group_members) — die Lernreisen-Zuordnungen der Studenten
// (learning_journey_assignments) bleiben unberührt. In der Zuordnungstabelle
// erscheint danach bei den betroffenen Studenten "keine Lerngruppe".
async function deleteLearningGroup(id){
  const g=(TONI_GROUPS||[]).find(x=>String(x.id)===String(id)); if(!g)return;
  const ok=window.confirm(
    `Lerngruppe „${g.name}" wirklich löschen?\n\n`+
    `• Die Gruppenzuordnung wird allen Mitgliedern entzogen.\n`+
    `• Bereits zugeordnete Lernreisen bleiben bestehen.\n\n`+
    `Dies kann nicht rückgängig gemacht werden.`
  );
  if(!ok)return;
  try{
    // 1) Mitgliedschaften der Gruppe entfernen (nur Mitgliedschaft, nicht die Lernreisen).
    await supabaseRequest(`learning_group_members?group_id=eq.${encodeURIComponent(id)}`,{method:"DELETE"});
    // 2) Die Gruppe selbst löschen.
    await supabaseRequest(`learning_groups?id=eq.${encodeURIComponent(id)}`,{method:"DELETE"});
    if(String(ACTIVE_GROUP_ID)===String(id)){
      ACTIVE_GROUP_ID=null;
      const det=document.getElementById("group-detail"); if(det)det.style.display="none";
      const empty=document.getElementById("group-detail-empty"); if(empty)empty.style.display="";
    }
    await loadTutorGroups();
  }catch(e){
    console.warn("Lerngruppe löschen fehlgeschlagen:",e);
    alert("Die Lerngruppe konnte nicht gelöscht werden:\n"+(e.message||e));
  }
}
...
- [Pflicht] Lerninhalt | Stromstärke und Spannung erklären | Erkläre die Begriffe U und I mit eigenen Worten.
...
function rowToJourneyV16(row){
  ...
  return {
    ...
    cover_image: j.cover_image || row.cover_image || "",
...
function toniSlugifyTitle(title){
  return String(title || "lernreise")
    .toLowerCase()
    .replace(/[äöü]/g, m => ({"ä":"ae","ö":"oe","ü":"ue"}[m]))
    .replace(/ß/g, "ss")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .slice(0, 60) || "lernreise";
}

// Lädt eine URL und wandelt den Inhalt in eine Data-URL (Base64) um. Bei Fehler: null.
function toniUrlToDataUrl(url){
...
// Bettet alle Storage-Bilder/-Dateien einer Lernreise ein (für autarke Exportdatei).
// Gibt {journey, embeddedCount, failedCount} zurück. Schlägt nie hart fehl.
async function toniEmbedJourneyMedia(journey){
  const j = JSON.parse(JSON.stringify(journey || {}));
  let embeddedCount = 0, failedCount = 0;

  for(const step of (j.steps || [])){
    for(const task of (step.tasks || [])){
      if(!Array.isArray(task.blocks)) continue;
      for(const block of task.blocks){
        // Nur Bilder/Dateien mit echter http-URL einbetten; Links/YouTube bleiben unangetastet
        const isMedia = block && (block.type === "image" || block.type === "file");
        const url = block && block.url;
        if(isMedia && typeof url === "string" && /^https?:\/\//.test(url) && !block.embedded){
          const dataUrl = await toniUrlToDataUrl(url);
          if(dataUrl){
            block.embedded = dataUrl;
            block.original_url = url; // Herkunft merken
            embeddedCount++;
          }else{
            failedCount++; // URL bleibt erhalten, Bild evtl. später nicht verfügbar
          }
        }
      }
    }
  }
  return { journey: j, embeddedCount, failedCount };
}

async function toniExportJourney(id){
  if(typeof canManageLearningJourneysV16 === "function" && !canManageLearningJourneysV16()){
    alert("Nur Admins und Tutoren können Lernreisen exportieren.");
    return;
  }
  ...
  try{
    const baseJourney = rowToJourneyV16(row);

    appendMsg?.("toni", "📦 Material wird für den Export eingebettet …", typeof time === "function" ? time() : "", "desktop");
    const { journey, embeddedCount, failedCount } = await toniEmbedJourneyMedia(baseJourney);

    const paket = {
      format: "toni-lernreise",
      format_version: 1,
      exported_at: new Date().toISOString(),
      exported_by_role: (typeof getCurrentRole === "function" ? getCurrentRole() : "tutor"),
      journey
    };
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${toniSlugifyTitle(journey.title)}.toni.json`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);

    let msg = `📤 Lernreise <strong>${escapeHtml(journey.title)}</strong> wurde exportiert.`;
    if(embeddedCount > 0) msg += ` ${embeddedCount} Bild(er)/Datei(en) eingebettet.`;
    if(failedCount > 0) msg += ` ⚠️ ${failedCount} Material-Datei(en) konnten nicht eingebettet werden (Verweis bleibt erhalten).`;
    appendMsg?.("toni", msg, typeof time === "function" ? time() : "", "desktop");
  }catch(error){
    console.error("Lernreise exportieren:", error);
    alert("Lernreise konnte nicht exportiert werden:\n" + (error.message || error));
  }
}

/* TONI – Lernreise importieren (Schritt 3a: Datei lesen, prüfen, Vorschau) */

// Prüft ein eingelesenes Paket auf gültiges TONI-Format. Gibt {ok, error, journey} zurück.
function toniValidateImportPaket(paket){
  if(!paket || typeof paket !== "object"){
    return { ok:false, error:"Die Datei ist keine gültige TONI-Lernreise." };
  }
  if(paket.format !== "toni-lernreise"){
    return { ok:false, error:"Diese Datei ist keine TONI-Lernreise (falsches Format)." };
  }
  if(typeof paket.format_version !== "number" || paket.format_version > 1){
    return { ok:false, error:"Diese Datei wurde mit einer neueren TONI-Version erstellt und kann nicht gelesen werden." };
  }
  const j = paket.journey;
  if(!j || typeof j !== "object" || !j.title || !Array.isArray(j.steps)){
    return { ok:false, error:"Die Lernreise in der Datei ist unvollständig oder beschädigt." };
  }
  return { ok:true, journey:j };
}

// Zählt Stationen, Aufgaben und einzubettende Medien für die Vorschau.
function toniImportStats(journey){
  let stations = (journey.steps || []).length;
  let tasks = 0, media = 0;
  for(const s of (journey.steps || [])){
    for(const t of (s.tasks || [])){
      tasks++;
      for(const b of (t.blocks || [])){
        if(b && (b.type === "image" || b.type === "file") && (b.embedded || b.url)) media++;
      }
    }
  }
  const hasCover = !!(journey.cover_image || journey.cover_image_embedded);
  return { stations, tasks, media, hasCover };
}

// Hält das aktuell eingelesene Paket bis zur Bestätigung des Imports vor.
window.TONI_IMPORT_PENDING = null;

async function toniHandleImportFile(event){
  const input = event?.target;
  const file = input?.files?.[0];
  const status = document.getElementById("journey-import-status");
  if(input) input.value = ""; // erlaubt erneute Auswahl derselben Datei

  if(typeof canManageLearningJourneysV16 === "function" && !canManageLearningJourneysV16()){
    alert("Nur Admins und Tutoren können Lernreisen importieren.");
    return;
  }
  if(!file) return;

  if(status) status.textContent = "Datei wird gelesen …";
  ...
  try{
    ...
    alert("Die Datei konnte nicht gelesen werden. Ist es eine gültige TONI-Lernreise (.toni.json)?");
    return;
  }

  const check = toniValidateImportPaket(paket);
  if(!check.ok){
    if(status) status.textContent = "";
    alert(check.error);
    return;
  ...
  window.TONI_IMPORT_PENDING = check.journey;
  const st = toniImportStats(check.journey);
  if(status) status.textContent = "";

  // Vorschau zeigen und Bestätigung einholen (Schritt 3b führt den eigentlichen Import aus)
  const meldung = `Importvorschau:\n\n` +
    `Titel: ${check.journey.title}\n` +
    `Fach: ${check.journey.subject || "—"}\n` +
    `Stationen: ${st.stations}\n` +
    `Aufgaben: ${st.tasks}\n` +
    `Material (Bilder/Dateien): ${st.media}${st.hasCover ? "\nDeckblatt: vorhanden" : ""}\n\n` +
    `Als neue Lernreise importieren?`;

  if(confirm(meldung)){
    if(typeof toniPerformImport === "function"){
      await toniPerformImport(check.journey);
    }else{
      alert("Import wird in Kürze verfügbar sein.");
    }
  }else{
    window.TONI_IMPORT_PENDING = null;
  }
...
/* TONI – Lernreise importieren (Schritt 3b: Medien ins Storage, Eigentümer, speichern) */

// Wandelt eine Data-URL (Base64) zurück in ein Blob.
function toniDataUrlToBlob(dataUrl){
  try{
...
// Lädt ein Blob in den Storage der eigenen Institution und gibt die öffentliche URL zurück.
async function toniUploadBlobToStorage(blob, baseName, kind){
  const token = typeof toniV27GetAccessToken === "function" ? await toniV27GetAccessToken() : null;
  const safe = String(baseName || "datei").replace(/[^a-zA-Z0-9._-]/g, "_");
  const path = (kind === "file" ? `tasks/files/${Date.now()}_${safe}` : `tasks/img_${Date.now()}_${safe}`);
  const res = await fetch(`${window.SUPABASE_URL}/storage/v1/object/learning-content/${path}`, {
    method: "POST",
    headers: { "apikey": window.SUPABASE_ANON_KEY, "Authorization": "Bearer " + token, "Content-Type": blob.type || "application/octet-stream", "x-upsert": "true" },
    body: blob
  });
  if(!res.ok) throw new Error(await res.text());
  return `${window.SUPABASE_URL}/storage/v1/object/public/learning-content/${path}`;
}

async function toniPerformImport(journey){
  const status = document.getElementById("journey-import-status");
  try{
    if(typeof canManageLearningJourneysV16 === "function" && !canManageLearningJourneysV16()){
      alert("Nur Admins und Tutoren können Lernreisen importieren.");
    ...
    const j = JSON.parse(JSON.stringify(journey)); // unabhängige Kopie
    let uploaded = 0, kept = 0;

    // 1. Material-Bilder/-Dateien: eingebettete Werte in den eigenen Storage hochladen
    for(const step of (j.steps || [])){
      for(const task of (step.tasks || [])){
        if(!Array.isArray(task.blocks)) continue;
        for(const block of task.blocks){
          if(!block || (block.type !== "image" && block.type !== "file")) continue;
          const src = block.embedded || (typeof block.url === "string" && block.url.startsWith("data:") ? block.url : "");
          if(!src) { if(block.url) kept++; continue; } // nur URL vorhanden -> unverändert lassen
          if(status) status.textContent = "Material wird hochgeladen …";
          const blob = toniDataUrlToBlob(src);
          if(!blob){ kept++; continue; }
          try{
            const name = block.name || block.alt || (block.type === "file" ? "datei" : "bild");
            const newUrl = await toniUploadBlobToStorage(blob, name, block.type);
            block.url = newUrl;
            delete block.embedded;
            delete block.original_url;
            uploaded++;
          }catch(e){
            // Hochladen fehlgeschlagen: eingebettet lassen, damit das Bild nicht verloren geht
            block.url = src;
            kept++;
          }
        }
    ...
    // 2. Deckblatt: eingebettetes Cover in den Storage hochladen
    const coverEmbedded = j.cover_image_embedded || (typeof j.cover_image === "string" && j.cover_image.startsWith("data:") ? j.cover_image : "");
    if(coverEmbedded){
      if(status) status.textContent = "Deckblatt wird hochgeladen …";
      const blob = toniDataUrlToBlob(coverEmbedded);
      if(blob){
        try{
          const newUrl = await toniUploadBlobToStorage(blob, "cover_" + toniSlugifyTitle(j.title), "image");
          j.cover_image = newUrl;
          j.cover_image_path = "";
          j.cover_image_embedded = "";
          uploaded++;
        }catch(e){
          // Fallback: Cover eingebettet lassen
          j.cover_image = coverEmbedded;
          j.cover_image_embedded = coverEmbedded;
        }
      }
    }

    // 3. Neue IDs vergeben, damit nichts Bestehendes überschrieben wird
    const newId = (typeof uuidLikeV16 === "function") ? uuidLikeV16() : ("imp-" + Date.now());
    j.id = newId;
    (j.steps || []).forEach((s, si) => {
      s.id = "station-" + (si + 1) + "-" + (typeof uuid === "function" ? uuid().slice(0,8) : Date.now());
      (s.tasks || []).forEach(t => { t.id = "task-" + (typeof uuid === "function" ? uuid() : Date.now() + Math.random()); });
    });

    // 4. Eigentümer = importierender Tutor; als neue Lernreise speichern
    ...
    if(status) status.textContent = "Lernreise wird gespeichert …";
    ...
    if(typeof supabaseRequest === "function"){
      ...
      try{
        await supabaseRequest("rpc/upsert_learning_journey_template_v44", {
          ...
          body: JSON.stringify({ p_payload: {
            id: rowData.id, title: rowData.title, subject: rowData.subject,
            goal: rowData.goal, description: rowData.description,
            journey_json: rowData.journey_json, owner_profile_id: ownerId
          }})
        });
        saved = true;
      }catch(rpcError){
        // Fallback: direkter Tabellenzugriff
        await supabaseRequest("learning_journey_templates", {
          method: "POST",
          headers: { "Prefer": "return=representation" },
          body: JSON.stringify([rowData])
      ...
      rows.unshift({ ...rowData, _source: "local", created_at: new Date().toISOString() });
      setLocalJourneysV16?.(rows);
    }
    ...
    let msg = `📥 Lernreise <strong>${escapeHtml(j.title)}</strong> wurde importiert.`;
    if(uploaded > 0) msg += ` ${uploaded} Bild(er)/Datei(en) in den Speicher übernommen.`;
    if(kept > 0) msg += ` ${kept} Material-Verweis(e) beibehalten.`;
    appendMsg?.("toni", msg, typeof time === "function" ? time() : "", "desktop");
  }catch(error){
    console.error("Lernreise importieren:", error);
    if(status) status.textContent = "";
    alert("Lernreise konnte nicht importiert werden:\n" + (error.message || error));
  }
}
...
function clearTaskBuilder(){const title=document.getElementById("task-title"),desc=document.getElementById("task-description"),type=document.getElementById("task-type"),req=document.getElementById("task-required");if(title)title.value="";if(desc)desc.value="";if(type)type.value="Info";if(req)req.checked=true}
...
// V17-Speicher-Wrapper entfernt 01.06.2026 – redundant: V44 führt syncJourneyBuilderToLegacyTextareaV17()
// und die Stations-Prüfung selbst aus, und V43/V44 ersetzen die Funktion ohnehin komplett. Säule 2 / 2.4.
...
/* TONI V25 – stillgelegt (V53 rendert die Timeline). Nur der No-Op-Builder bleibt,
   da updateLearningJourneyBar ihn noch aufruft. */
(function(){
  function toniV25BuildJourneyBar(){
    // Stillgelegt: V53 (toniV53BuildJourneyBar) hat diesen Builder fachlich ersetzt
    // (Start-/Ziel-Symbole, kein "Du bist hier"-Badge, Signatur-Guard). V25 und V53
    // schrieben beide dasselbe Element (.lernreise-wrap) -> Zittern. Nur noch V53 rendert.
    return;
  }
...
function toniV40CanPersist(){
  const role = toniV40Role();
  const isStaff = role === "tutor" || role === "admin" || role === "superadmin";
...
async function toniV40SaveActiveProgressNow(showIndicator=false){
  ...
  if(saveKey === window.TONI_V40_LAST_SAVE_KEY){
    return;
  }
...
// V43-Speicherfunktion (toniV43SaveLearningJourneyViaRpc) entfernt 01.06.2026 –
// vollständig durch V44 ersetzt (nutzte die alte RPC _v43). Säule 2 / Teilschritt 2.4.
...
(function(){
  ...
  // (V91-Wrapper entfernt 01.06.2026 – wurde durch spätere Neuzuweisungen überschrieben,
  //  Cover-Felder werden in der finalen Bau-Funktion direkt gesetzt. Säule 2 / Teilschritt 2.3.)
...
(function(){
  ...
  function dataUrlFromImage(file, maxSize = 1600, quality = 0.82){
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
  ...
  // (V92-Build-Wrapper entfernt 01.06.2026 – wurde durch spätere Neuzuweisungen
  //  überschrieben; Cover-Felder setzt die finale Bau-Funktion direkt. Säule 2 / 2.3.)
...
(function(){
  ...
  // (Tote Bau-Funktion entfernt 01.06.2026 – durch spätere Neuzuweisung überschrieben. Säule 2 / 2.3.)
...
(function(){
  ...
  // (Tote Bau-Funktion entfernt 01.06.2026 – durch die finale Neuzuweisung überschrieben. Säule 2 / 2.3.)
...
/* ============================================================
 * TONI V109 – Reset als letzte Station
 * ------------------------------------------------------------
 * Hängt ans Ende der Stationsliste (#lr-stations) eine eigene
 * "Reset-Station" an, die den vorhandenen, passwortgeschützten
 * Reset-Flow (toniV99OpenRestartPasswordModal) auslöst.
 *
 * Eigenschaften (laut Vorgabe):
 *  - passwortgeschützt für alle (nutzt bestehenden Flow)
 *  - alter Text-Link "Lernreise neu starten" wird entfernt
 *  - nur sichtbar, wenn Fortschritt > 0 %
 *
 * Theme-unabhängig: Wird NACH dem Stations-Rendering angehängt,
 * egal welches Theme (classic/space/football/...) die Stationen
 * selbst erzeugt. Greift NICHT in die einzelnen Theme-Renderer ein.
 * ============================================================ */
(function(){
  "use strict";

  var STYLE_ID = "toni-v109-reset-station-style";
  var STATION_ID = "toni-v109-reset-station";
  ...
  function injectStyleOnce(){
    ...
    var css = ''
      ...
      + '  box-shadow:0 2px 10px rgba(176,0,0,.08);'
      + '  transition:transform .12s ease,box-shadow .12s ease,background .12s ease;'
      + '}'
      + '#' + STATION_ID + ':hover{transform:translateY(-1px);box-shadow:0 4px 16px rgba(176,0,0,.16);background:linear-gradient(135deg,#ffefef,#ffd6d6);}'
      + '#' + STATION_ID + ' .v109-ico{'
      + '  flex:0 0 auto;width:44px;height:44px;border-radius:50%;'
      + '  display:flex;align-items:center;justify-content:center;'
      + '  background:#fff;border:1.5px solid #e89a9a;font-size:22px;'
      + '}'
      + '#' + STATION_ID + ' .v109-txt{flex:1 1 auto;min-width:0;}'
      + '#' + STATION_ID + ' .v109-title{font-weight:700;color:#9b1c1c;font-size:15px;}'
      + '#' + STATION_ID + ' .v109-sub{color:#a14b4b;font-size:13px;margin-top:2px;}'
      + '#' + STATION_ID + ' .v109-arrow{flex:0 0 auto;color:#c0392b;font-size:18px;font-weight:700;}';
  ...
  // Aktuellen Fortschritt der aktiven Reise ermitteln (robust, mit Fallbacks).
  ...
  // Alten Text-Link "Lernreise neu starten" entfernen (durch Station ersetzt).
  function removeOldRestartLink(){
    document.querySelectorAll('#lr-modal .v98-restart-link, #lr-modal .card-link[onclick*="toniV99OpenRestartPasswordModal"], #lr-modal .card-link[onclick*="toniV98RestartLearningJourney"]').forEach(function(link){
      // nur den dedizierten Restart-Link entfernen, nichts anderes
      link.remove();
    });
  }
  ...
  function triggerReset(){
    if(typeof window.toniV99OpenRestartPasswordModal === "function"){
      ...
      console.warn("[TONI V109] Kein Reset-Flow verfügbar.");
    }
  }

  function ensureResetStation(){
    var host = document.getElementById("lr-stations");
    if(!host) return;

    // Nur anzeigen, wenn das Lernreise-Modal offen ist.
    var modal = document.getElementById("lr-modal");
    if(modal && !modal.classList.contains("open")){
      var existing0 = document.getElementById(STATION_ID);
    ...
    var pct = currentProgressPercent();
    var existing = document.getElementById(STATION_ID);

    // Nur bei Fortschritt > 0 % anzeigen.
    if(pct <= 0){
    ...
    if(existing){
      // Schon vorhanden: nur sicherstellen, dass sie das letzte Element ist.
      if(existing.parentElement === host && host.lastElementChild !== existing){
    ...
    card.setAttribute("role", "button");
    card.setAttribute("tabindex", "0");
    card.setAttribute("aria-label", "Lernreise zurücksetzen");
    card.innerHTML = ''
      + '<div class="v109-ico">🔄</div>'
      + '<div class="v109-txt">'
      + '  <div class="v109-title">Lernreise zurücksetzen</div>'
      + '  <div class="v109-sub">Setzt deinen Fortschritt auf 0 %. Passwort erforderlich.</div>'
      + '</div>'
      + '<div class="v109-arrow">→</div>';
    card.addEventListener("click", function(ev){ ev.preventDefault(); triggerReset(); });
    card.addEventListener("keydown", function(ev){
      if(ev.key === "Enter" || ev.key === " "){ ev.preventDefault(); triggerReset(); }
    });

    host.appendChild(card);
  }

  // Wrapper um renderLearningJourneyModal: nach jedem Render die Station anhängen.
  function wrapRender(){
    if(typeof window.renderLearningJourneyModal === "function" && !window.renderLearningJourneyModal.__toniV109Wrapped){
      var original = window.renderLearningJourneyModal;
      window.renderLearningJourneyModal = function(){
        var result = original.apply(this, arguments);
        // nach dem (theme-spezifischen) Stations-Rendering anhängen
        setTimeout(ensureResetStation, 0);
        setTimeout(ensureResetStation, 60);
        return result;
      };
      window.renderLearningJourneyModal.__toniV109Wrapped = true;
    }
  }
  ...
  window.addEventListener("DOMContentLoaded", function(){
    ...
    // Theme-Re-Renders / DOM-Wechsel abfangen, damit die Station erhalten bleibt.
    var observer = new MutationObserver(function(){
      clearTimeout(window.TONI_V109_TIMER);
      window.TONI_V109_TIMER = setTimeout(function(){
        wrapRender();
        ensureResetStation();
      }, 90);
    });
    var stationsHost = document.getElementById("lr-stations") || document.body;
    observer.observe(document.body, { childList:true, subtree:true });
  });
})();
