...
function toniRenderAufgabe(task,el){
  ...
  const unit=task.expected_unit?` <span style="color:var(--color-text-tertiary);font-size:13px">${toniEsc(task.expected_unit)}</span>`:'';
  task._aufgabeTries=task._aufgabeTries||0;

  let html=`<div style="font-size:14px;color:var(--color-text-primary);line-height:1.7;margin-bottom:12px">${toniEsc(prompt)}</div>${media}`;

  if(hasCheck){
    html+=`<div style="background:var(--color-background-secondary);border-radius:10px;padding:12px 14px;margin-bottom:12px">
      <div style="font-size:12px;font-weight:600;color:var(--color-text-secondary);margin-bottom:8px">🎯 Dein Ergebnis prüfen</div>
      <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <input id="aufgabe-answer" inputmode="decimal" placeholder="Ergebnis" style="width:110px;padding:8px 10px;border:0.5px solid var(--color-border-secondary);border-radius:8px;font-size:14px;text-align:center" onkeydown="if(event.key==='Enter')toniCheckAufgabe()">
        ${unit}
        <button onclick="toniCheckAufgabe()" style="padding:8px 16px;background:#185FA5;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer">Prüfen</button>
        <span id="aufgabe-feedback" style="font-size:13px;font-weight:500"></span>
      </div>
    </div>`;
  }

  if(hasSolution){
    html+=`<div id="aufgabe-solution-wrap" style="border:0.5px solid var(--color-border-tertiary);border-radius:10px;overflow:hidden;margin-bottom:4px">
      <div id="aufgabe-solution-toggle" onclick="toniToggleSolution()" style="padding:11px 14px;background:var(--color-background-secondary);font-size:13px;font-weight:500;color:#854F0B;cursor:pointer;display:flex;justify-content:space-between;align-items:center">
        <span>💡 Musterlösung anzeigen</span><span id="aufgabe-solution-hint" style="font-size:11px;color:var(--color-text-tertiary)">erst nach einem Versuch</span>
      </div>
      <div id="aufgabe-solution-body" style="display:none;padding:13px 14px;font-size:13px;color:var(--color-text-primary);line-height:1.6;border-top:0.5px solid var(--color-border-tertiary)">
        ${toniEsc(task.solution).replace(/\n/g,'<br>')}
        <div id="aufgabe-selfcheck" style="margin-top:14px;padding-top:12px;border-top:0.5px solid var(--color-border-tertiary)">
          <div style="font-size:12px;font-weight:600;color:var(--color-text-secondary);margin-bottom:8px">Wie gut passt deine Lösung?</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button onclick="toniAufgabeSelfAssess('stimmt')" style="padding:7px 14px;border:0.5px solid #639922;background:#EAF3DE;color:#27500A;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer">✅ Stimmt</button>
            <button onclick="toniAufgabeSelfAssess('teilweise')" style="padding:7px 14px;border:0.5px solid #B5841F;background:#FAEEDA;color:#633806;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer">🟡 Teilweise</button>
            <button onclick="toniAufgabeSelfAssess('nochmal')" style="padding:7px 14px;border:0.5px solid #C0563A;background:#FAECE7;color:#712B13;border-radius:8px;font-size:12px;font-weight:500;cursor:pointer">🔁 Nochmal ansehen</button>
          </div>
          <div id="aufgabe-selfcheck-fb" style="font-size:12px;color:var(--color-text-tertiary);margin-top:8px"></div>
        </div>
      </div>
    </div>`;
  }
