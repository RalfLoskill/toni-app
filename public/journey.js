...
function renderAdminJourneyListV16(rows, listElId){
  ...
  const tiles = rows.map(row => {
    ...
    const actionsHtml = restrictToAssigned
      ? `<button class="journey-tile-action" onclick="duplicateAdminJourney('${row.id}')" title="Duplizieren" aria-label="Duplizieren"><span>📑</span>Duplizieren</button>
          <button class="journey-tile-action danger" onclick="removeAssignedJourneyForStudent('${row.id}', this)" data-journey-title="${escapeToniV16(j.title)}" title="Aus meiner Liste entfernen" aria-label="Aus meiner Liste entfernen"><span>🗑️</span>Entfernen</button>`
