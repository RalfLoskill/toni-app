...
// Senkrechte Reiseroute mit anklickbaren Aufgaben je Etappe (Säule 3)
function toniRenderJourneyTimeline(j){
  ...
  return `<div class="toni-tl">`+steps.map((s,i)=>{
    ...
    // Verbindungslinie (außer letzter)
    ...
    const tasksHtml=(s.tasks||[]).map(t=>{
      const icon=toniTypeIcon(toniNormalizeType(t.type));
      if(locked){
        return `<div class="toni-tl-task locked"><span>🔒</span><span class="toni-tl-tname">${toniEsc(t.title)}</span></div>`;
      }
      const statusIcon=t.status==='done'?'<span style="color:#639922">✅</span>':t.status==='in_progress'?'<span>🟡</span>':`<span>${icon}</span>`;
      const isCurrentTask=(st==='current'&&t.status!=='done');
      const meta=t.status==='done'?'<span class="toni-tl-meta">erledigt</span>':isCurrentTask?'<span class="toni-tl-meta cur">jetzt dran</span>':`<span class="toni-tl-meta">${t.required?'Pflicht':'optional'}</span>`;
      return `<div class="toni-tl-task ${isCurrentTask?'current':''}" onclick="openLearningTask('${t.id}')">${statusIcon}<span class="toni-tl-tname ${t.status==='done'?'done':''}">${toniEsc(t.title)}</span>${meta}<span class="toni-tl-arr">→</span></div>`;
    }).join('');
    const subLabel = locked
      ? `<div class="toni-tl-sub">${(s.tasks||[]).length} Aufgabe(n) · gesperrt</div>`
      : (s.subtitle?`<div class="toni-tl-sub">${toniEsc(s.subtitle)}</div>`:'');
    return `<div class="toni-tl-step">
      <div class="toni-tl-rail">
        <div class="toni-tl-dot ${dotCls}">${dotContent}</div>
        ${connector}
      </div>
      <div class="toni-tl-body">
        <div class="toni-tl-head ${st}">${toniEsc(s.title)} ${!locked&&req.length?`<span class="toni-tl-count">${done}/${req.length}</span>`:''}</div>
        ${subLabel}
        <div class="toni-tl-tasks">${tasksHtml}</div>
      </div>
    </div>`;
  }).join('')+`</div>`;
}
