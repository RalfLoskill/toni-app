...
function openLearningTask(id){
  ...
  switch(normType){
    ...
    case 'Lerninhalt':
      toniRenderLerninhalt(task, contentEl);
      if(answerEl) answerEl.style.display='none';
      break;

    case 'Video':
      toniRenderVideo(task, contentEl);
      if(answerEl) answerEl.style.display='none';
      break;

    case 'Quiz':
      toniRenderQuiz(task, contentEl);
      if(answerEl) answerEl.style.display='none';
      break;

    case 'Reflexion':
      toniRenderReflexion(task, contentEl);
      if(answerEl) answerEl.style.display='none';
      break;

    default: // Aufgabe + alte Typen
...
// ══════════════════════════════════════════════════════════
// TONI – Aufgabentyp Hilfsfunktionen (Schritt 1: Fundament)
// ══════════════════════════════════════════════════════════
...
function toniTypeIcon(type){
  const icons={'Lerninhalt':'📖 ','Aufgabe':'✏️ ','Quiz':'🎯 ','Video':'🎬 ','Reflexion':'💬 '};
  return icons[type]||'📌 ';
}

function toniTypeBg(type){
  const m={'Lerninhalt':'#E6F1FB','Aufgabe':'#EAF3DE','Quiz':'#FAEEDA','Video':'#EEEDFE','Reflexion':'#FBEAF0'};
  return m[type]||'var(--color-background-secondary)';
}

function toniTypeColor(type){
  const m={'Lerninhalt':'#0C447C','Aufgabe':'#27500A','Quiz':'#633806','Video':'#3C3489','Reflexion':'#72243E'};
  return m[type]||'var(--color-text-primary)';
}

function toniTaskPrompt(type){
  const m={'Lerninhalt':'Lies den Inhalt aufmerksam durch und klicke danach auf "Gelesen – weiter".','Aufgabe':'Bearbeite die Aufgabe und gib deine Antwort ein.','Quiz':'Beantworte die Fragen. Du bekommst sofort Feedback.','Video':'Schau das Video und notiere dir drei Kernaussagen.','Reflexion':'Nimm dir kurz Zeit zum Nachdenken. Es gibt keine falsche Antwort.'};
  return m[type]||'Bearbeite die Aufgabe Schritt für Schritt.';
}
...
function toniRenderLerninhalt(task, el){
  ...
  // blocks-Array verarbeiten (neues Format)
  const blocks=task.blocks||task.lerninhalt_blocks;
  if(Array.isArray(blocks)){
    const raw=typeof blocks==='string'?JSON.parse(blocks):blocks;
    raw.forEach(b=>{
...
function toniRenderVideo(task, el){
  ...
  const links=Array.isArray(blocks)?blocks.filter(b=>b.type==='link'):[];
  if(task.links) links.push(...(task.links||[]));

  if(ytId){
    el.innerHTML=`
      <div style="margin-bottom:12px">
        <div style="position:relative;padding-bottom:56.25%;height:0;overflow:hidden;border-radius:10px;background:#000">
          <iframe src="https://www.youtube.com/embed/${toniEsc(ytId)}" frameborder="0" allowfullscreen
            style="position:absolute;top:0;left:0;width:100%;height:100%"></iframe>
        </div>
      </div>
      ${desc?`<div style="font-size:13px;color:var(--color-text-secondary);line-height:1.6;margin-bottom:10px">${toniEsc(desc)}</div>`:''}
      <div style="background:#FAEEDA;border-radius:8px;padding:8px 12px;font-size:12px;color:#633806">
        💡 Tipp: Notiere nach dem Video drei Kernaussagen.
      </div>`;
  } else {
    el.innerHTML=`<div style="font-size:14px;color:var(--color-text-primary)">${toniEsc(desc)}</div>
      ${links.map(l=>`<a href="${toniEsc(l.url)}" target="_blank" style="display:flex;align-items:center;gap:8px;margin-top:10px;padding:10px;background:#EEEDFE;border-radius:8px;text-decoration:none;color:#3C3489"><span style="font-size:18px">▶️</span><span>${toniEsc(l.title||l.url)}</span></a>`).join('')}`;
  }
}
...
function toniRenderQuiz(task, el){
  ...
  if(!qdata||!qdata.questions?.length){
    el.innerHTML=`<div style="font-size:14px;color:var(--color-text-primary)">${toniEsc(task.content||task.description||'')}</div>
      <div style="margin-top:10px;background:#FAEEDA;border-radius:8px;padding:8px 12px;font-size:12px;color:#633806">🎯 Quiz-Fragen werden noch vom Tutor erstellt.</div>`;
    return;
  }
  const q=qdata.questions[task._quizIndex||0];
  const total=qdata.questions.length;
  const idx=(task._quizIndex||0)+1;
  el.innerHTML=`
    <div style="font-size:12px;color:var(--color-text-tertiary);margin-bottom:8px">Frage ${idx} von ${total}</div>
    <div style="font-size:14px;font-weight:500;color:var(--color-text-primary);margin-bottom:12px">${toniEsc(q.question)}</div>
    <div id="quiz-options">
      ${q.options.map((opt,i)=>`
        <button onclick="toniQuizAnswer(${i})" style="display:block;width:100%;text-align:left;padding:10px 14px;margin-bottom:6px;background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;font-size:13px;cursor:pointer;color:var(--color-text-primary)">
          ${String.fromCharCode(65+i)}. ${toniEsc(opt)}
        </button>`).join('')}
    </div>`;
}
...
window.toniQuizAnswer=function(idx){
  ...
  opts.forEach((btn,i)=>{
    btn.style.background=i===correct?'#EAF3DE':i===idx?'#FCEBEB':'var(--color-background-secondary)';
    btn.style.borderColor=i===correct?'#639922':i===idx&&idx!==correct?'#E24B4A':'var(--color-border-tertiary)';
    btn.style.color=i===correct?'#27500A':i===idx&&idx!==correct?'#791F1F':'var(--color-text-primary)';
    btn.disabled=true;
  });
  const feedback=document.createElement('div');
  feedback.style.cssText='margin-top:10px;padding:10px;border-radius:8px;font-size:13px';
  feedback.style.background=idx===correct?'#EAF3DE':'#FCEBEB';
  feedback.style.color=idx===correct?'#27500A':'#791F1F';
  feedback.innerHTML=idx===correct?'✅ Richtig! '+toniEsc(q.explanation||''):'❌ Leider falsch. '+toniEsc(q.explanation||'Die richtige Antwort ist: '+q.options[correct]);
  document.getElementById('quiz-options').appendChild(feedback);
};
...
function toniRenderReflexion(task, el){
  ...
  el.innerHTML=`
    <div style="font-size:14px;color:var(--color-text-primary);line-height:1.7;margin-bottom:12px">${toniEsc(prompt)}</div>
    <div style="margin-bottom:12px">
      <div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:6px">Wie gut verstehst du das Thema?</div>
      <div style="display:flex;gap:6px" id="reflexion-stars">
        ${[1,2,3,4,5].map(i=>`<button onclick="toniReflexionStar(${i})" style="font-size:20px;background:none;border:none;cursor:pointer;opacity:.4" data-star="${i}">⭐</button>`).join('')}
      </div>
    </div>`;
  // Antwortfeld bleibt erhalten (lr-answer)
  const answerEl=document.getElementById('lr-answer');
  if(answerEl){ answerEl.style.display=''; answerEl.placeholder='Schreib deine Gedanken auf…'; }
}

window.toniReflexionStar=function(val){
  document.querySelectorAll('#reflexion-stars button').forEach((btn,i)=>{
...
(function(){
  ...
  function buildTaskFromVisibleForm(){
    ...
    // blocks-Array für Lerninhalt direkt anhängen
    const normT=toniNormalizeType(type);
    if(normT==='Lerninhalt'){
      const blocks=[
      ...
      if(blocks.length){
        ...
        window.TONI_LERNINHALT_FILES=[];
        window.TONI_LERNINHALT_LINKS=[];
        const status=document.getElementById('lerninhalt-save-status');
        if(status){ status.innerHTML=`✅ ${blocks.length} Medium/Medien gespeichert`; setTimeout(()=>status.innerHTML='',3000); }
...
/* ============================================================
   TONI – Lerninhalt Upload-Funktionen + Builder-Extras
   Schritt 1: Fundament
   ============================================================ */
(function(){
  ...
  window.toniHandleImgUpload = async function(input){
    ...
    if(preview) preview.innerHTML='<span style="font-size:11px;color:var(--color-text-tertiary)">Wird hochgeladen…</span>';
    try {
      ...
      if(preview) preview.innerHTML=`<img src="${url}" style="max-height:80px;border-radius:6px;border:0.5px solid var(--color-border-tertiary);margin-top:4px"><div style="font-size:11px;color:#27500A;margin-top:2px">✅ Hochgeladen</div>`;
    } catch(e){
      if(preview) preview.innerHTML=`<span style="color:#A32D2D;font-size:11px">Fehler: ${e.message}</span>`;
    }
  };
  ...
  window.toniHandleFileUpload = async function(input){
    ...
    if(list) list.innerHTML='<span style="font-size:11px;color:var(--color-text-tertiary)">Wird hochgeladen…</span>';
    try {
      const token=await getToken();
      ...
      if(list) list.innerHTML=`<span style="font-size:11px;color:#27500A">✅ ${file.name}</span>`;
    } catch(e){
      if(list) list.innerHTML=`<span style="color:#A32D2D;font-size:11px">Fehler: ${e.message}</span>`;
    }
  };
  ...
  window.toniAddLink = function(){
    ...
    if(list) list.innerHTML=window.TONI_LERNINHALT_LINKS.map(l=>`<div style="font-size:11px;color:var(--color-text-secondary);padding:2px 0">🔗 ${l.title}</div>`).join('');
  };

  // Builder-Extras für Lerninhalt einblenden
  function enhanceBuilder(){
    const sel=document.getElementById('task-type');
    if(!sel||sel.__toniEnhanced) return;
    sel.__toniEnhanced=true;

    const container=sel.closest('form')||sel.closest('[class*="modal"]')||sel.parentElement?.parentElement;
    if(!container) return;
    ...
    extras.innerHTML=`
      <div style="background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:12px;margin-top:8px">
        <div style="font-size:12px;font-weight:500;color:#185FA5;margin-bottom:10px">📖 Medien für Lerninhalt</div>
        <div style="margin-bottom:8px">
          <label style="font-size:11px;color:var(--color-text-tertiary);display:block;margin-bottom:3px">Bild hinzufügen</label>
          <input type="file" accept="image/*" onchange="toniHandleImgUpload(this)" style="font-size:12px;width:100%">
          <div id="lerninhalt-img-preview" style="margin-top:4px"></div>
        </div>
        <div style="margin-bottom:8px">
          <label style="font-size:11px;color:var(--color-text-tertiary);display:block;margin-bottom:3px">Datei-Anhang (PDF, Word…)</label>
          <input type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onchange="toniHandleFileUpload(this)" style="font-size:12px;width:100%">
          <div id="lerninhalt-file-list" style="margin-top:4px"></div>
        </div>
        <div>
          <label style="font-size:11px;color:var(--color-text-tertiary);display:block;margin-bottom:3px">Link hinzufügen</label>
          <div style="display:flex;gap:5px">
            <input type="url" id="lerninhalt-link-url" placeholder="https://…" style="flex:2;padding:5px 8px;border:0.5px solid var(--color-border-secondary);border-radius:6px;font-size:12px;background:var(--color-background-primary);color:var(--color-text-primary)">
            <input type="text" id="lerninhalt-link-title" placeholder="Titel" style="flex:1;padding:5px 8px;border:0.5px solid var(--color-border-secondary);border-radius:6px;font-size:12px;background:var(--color-background-primary);color:var(--color-text-primary)">
            <button type="button" onclick="toniAddLink()" style="padding:5px 10px;background:#185FA5;color:#fff;border:none;border-radius:6px;font-size:12px;cursor:pointer">+</button>
          </div>
          <div id="lerninhalt-links-list" style="margin-top:4px"></div>
        </div>
        <div id="lerninhalt-save-status" style="font-size:11px;color:#27500A;margin-top:6px;min-height:16px"></div>
      </div>`;

    const descField=container.querySelector('#task-description')?.closest('.lr-form-group');
    if(descField) descField.after(extras);
    else container.appendChild(extras);

    sel.addEventListener('change',()=>{
      extras.style.display=toniNormalizeType(sel.value)==='Lerninhalt'?'block':'none';
    });
    extras.style.display=toniNormalizeType(sel.value)==='Lerninhalt'?'block':'none';
  }

  const obs=new MutationObserver(()=>{
    if(document.getElementById('task-type')&&!document.getElementById('task-type').__toniEnhanced){
      enhanceBuilder();
    }
  });
  obs.observe(document.body,{childList:true,subtree:true});
  if(document.readyState!=='loading') enhanceBuilder();
  else document.addEventListener('DOMContentLoaded',enhanceBuilder);
