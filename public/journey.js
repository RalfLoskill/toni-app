...
function renderAdminJourneyListV16(rows){
  ...
  const tiles = rows.map(row => {
    ...
    const bgStyle = hasCover
      ? ` style="background-image:url('${escapeToniV16(coverImg)}')"`
      : "";
    return `
      <div class="journey-tile journey-tile-cover-v112 ${hasCover ? "has-cover" : "no-cover"}"${bgStyle}>
...
function toniV50ActivityJourneyHtml(journey){
  ...
  // Deckblatt-Hintergrundbild (optional). Ohne Bild: neutraler Fallback.
  const coverImg = journey.cover_image || journey.cover_image_embedded || "";
  const hasCover = !!coverImg;
  const bgStyle = hasCover
    ? ` style="background-image:url('${toniV50Escape(coverImg)}')"`
    : "";

  return `
    <div class="projekt-item activity-journey-item-v50 lr-cover-card-v110 ${hasCover ? "has-cover" : "no-cover"} ${active ? "active" : ""}"${bgStyle} onclick="toniV50OpenActivityJourney('${toniV50Escape(journey.id)}')">
...
(function(){
  ...
  function completedItemHtml(item, index){
    ...
    const coverImg = journey.cover_image || journey.cover_image_embedded || "";
    const hasCover = !!coverImg;
    const bgStyle = hasCover ? ` style="background-image:url('${esc(coverImg)}')"` : "";

    return `
      <div class="projekt-item activity-journey-item-v50 activity-completed-item-v83 activity-completed-item-v86 lr-cover-card-v110 ${hasCover ? "has-cover" : "no-cover"}"${bgStyle}
           role="button"
           tabindex="0"
