...
window.TONI_JOURNEY_BUILD = "v152-load-add-missing";
...
function rowToJourneyV16(row){
  ...
  return {
    ...
    theme: row.theme || j.theme || "classic",
    steps: j.steps || []
  };
}
...
function toniV24NormalizeTemplate(template){
  ...
  return {
    ...
    theme: template.theme || j.theme || "classic",
    steps: j.steps || []
  };
}
 ...
 * loadAssignedJourneysForStudentV20 ist die einzige Stelle, die zugewiesene
 * Reisen aus der DB holt, und wird von allen Lade-/Öffnen-Pfaden genutzt.
 * Wir wrappen sie: nach jedem Laden wird JEDE Reise im STATE mit dem frischen
 * Template abgeglichen (Inhalt+Theme frisch, Fortschritt per ID erhalten).
 * So ist der STATE aktuell, BEVOR irgendein Öffnen-Pfad rendert – unabhängig
 * von der vielfach überschriebenen openLearningJourney/render-Kette.
 * ============================================================ */
(function(){
  // Funktion ist eine freie Deklaration (nicht an window gebunden) -> via globalem
  // Zugriff wrappen. Im Browser sind frei und window identisch; hier robust beides prüfen.
  const ref = (typeof loadAssignedJourneysForStudentV20 === "function")
    ? loadAssignedJourneysForStudentV20
    : (typeof window !== "undefined" ? window.loadAssignedJourneysForStudentV20 : null);
  if(typeof ref !== "function" || ref.__toniFreshMergeWrapped) return;
  const wrapped = async function(){
    const rows = await ref.apply(this, arguments);
    try{
      if(Array.isArray(rows) && typeof STATE !== "undefined" && Array.isArray(STATE.learningJourneys)){
        rows.forEach(function(r){
          ...
          if(pos >= 0){
            ...
            // Reise ist zugewiesen, aber noch nicht im STATE -> frisch aufnehmen
            // (kein alter Fortschritt vorhanden, Template-Defaults gelten).
  ...
  wrapped.__toniFreshMergeWrapped = true;
  // an beide Bindungen schreiben: freie Variable UND window
  try{ loadAssignedJourneysForStudentV20 = wrapped; }catch(e){}
  if(typeof window !== "undefined") window.loadAssignedJourneysForStudentV20 = wrapped;
})();
