...
/* ============================================================
   ...
   Setzt Blocks VOR dem addStation-Schritt via mousedown
   ============================================================ */
(function(){
  // mousedown feuert VOR click und VOR stopImmediatePropagation
  document.addEventListener('mousedown', function(event){
    ...
    if(text.includes('aufgabe hinzufügen') || text.includes('aufgabe zur station')){
      const blocks = [
        ...(window.TONI_LERNINHALT_IMAGES||[]),
        ...(window.TONI_LERNINHALT_FILES||[]),
        ...(window.TONI_LERNINHALT_LINKS||[]),
      ...
      if(blocks.length){
        ...
        console.log('✅ TONI mousedown: Blocks vorgemerkt', blocks.length);
      }
    }

    // "Station hinzufügen" Button – Blocks bereits in Tasks
    if(text.includes('station zur lernreise') || text.includes('station aktualisieren')){
      // Alle Tasks mit pending blocks ausstatten
      if(window.TONI_PENDING_LERNINHALT_BLOCKS_V2?.length){
        const tasks = window.TONI_CURRENT_STATION_TASKS;
        if(tasks?.length){
          tasks[tasks.length-1].lerninhalt_blocks = window.TONI_PENDING_LERNINHALT_BLOCKS_V2;
          console.log('✅ TONI mousedown station: Blocks in Task injiziert');
        }
        window.TONI_PENDING_LERNINHALT_BLOCKS_V2 = null;
      }
    }
  }, true); // capture – feuert vor allem anderen

  // Nach click: Tasks in STATION_BUILDER aktualisieren
  document.addEventListener('click', function(event){
    const btn = event.target.closest?.('button');
    if(!btn) return;
    const text = (btn.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();
    ...
    if(text.includes('aufgabe hinzufügen') || text.includes('aufgabe zur station')){
      ...
      // Mit mehreren Delays versuchen
      [50, 150, 300, 500].forEach(delay => {
        setTimeout(() => {
          const tasks = window.TONI_CURRENT_STATION_TASKS;
          if(tasks?.length && !tasks[tasks.length-1].lerninhalt_blocks?.length){
            tasks[tasks.length-1].lerninhalt_blocks = [...blocks];
            console.log(`✅ TONI click delay ${delay}ms: Blocks injiziert`);
            window.TONI_LERNINHALT_IMAGES = [];
            window.TONI_LERNINHALT_FILES = [];
            window.TONI_LERNINHALT_LINKS = [];
          }
