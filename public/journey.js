...
window.TONI_JOURNEY_BUILD = "v86d-diagnose";
...
async function toniV41LoadAssignmentsWithProgress(){
  if(typeof supabaseRequest === "function"){
    try{
      ...
      // === TEMP-DIAGNOSE v86d (wird nach Befund entfernt) ===
      try {
        window.TONI_DIAG_RPC = {
          ok: true,
          count: (rows || []).length,
          firstKeys: rows && rows[0] ? Object.keys(rows[0]) : [],
          hasProgress: !!(rows && rows[0] && "progress_percent" in rows[0]),
          hasGroup: !!(rows && rows[0] && "student_group_name" in rows[0]),
          sampleGroup: rows && rows[0] ? rows[0].student_group_name : undefined,
          samplePct: rows && rows[0] ? rows[0].progress_percent : undefined,
        };
        console.log("TONI_DIAG_RPC:", JSON.stringify(window.TONI_DIAG_RPC));
      } catch(e){}
      // === /TEMP-DIAGNOSE ===
...
function toniV41StudentFromAssignment(a){
  ...
  // Erweiterte RPC liefert zusätzlich Lerngruppe und Fortschritt (LEFT JOINs).
  // Fallback bleibt leer/null, falls die alte RPC noch ohne diese Felder läuft.
