...
async function toniRequestOutline(){
  ...
  try{
    const resp = await fetch("/api/agent", {
      ...
      body: JSON.stringify({ agentType: "journey_outline", prompt, profileId: (window.TONI_AUTH_PROFILE && TONI_AUTH_PROFILE.id) || null })
    });
...
async function toniSubmitAiJourney(){
  ...
  try{
    const resp = await fetch("/api/agent", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ agentType: "journey_builder", prompt, depth, profileId: (window.TONI_AUTH_PROFILE && TONI_AUTH_PROFILE.id) || null })
    });
    const result = await resp.json().catch(() => ({}));
...
function toniSetJourneyEditorOpen(open){
  // Neu: Der Editor wird über die native Sektion "manage" auf-/zugeklappt
  // (schlichte Zeile "Lernreise erstellen/bearbeiten" mit + wie die anderen).
  try{
    if(window.TONI_SECTION_OPEN_STATE){
      window.TONI_SECTION_OPEN_STATE["manage"] = !!open;
      if(typeof toniV19ApplySectionState === "function"){
        toniV19ApplySectionState("journey-admin-panel","manage");
      }
    }
  }catch(e){}
  const card=toniGetJourneyEditorCard();
  if(card && open){
    try{ card.scrollIntoView({behavior:"smooth", block:"start"}); }catch(e){}
  }
}
function toniToggleJourneyEditor(){
  const cur = window.TONI_SECTION_OPEN_STATE ? !!window.TONI_SECTION_OPEN_STATE["manage"] : false;
  toniSetJourneyEditorOpen(!cur);
}
...
function toniV93RestructureEditor(){
  ...
  // 1) Kein eigener Kopf-Toggle mehr: Die native Sektion "Lernreise
  //    erstellen/bearbeiten" (#journey-admin-panel .card-header, toniV19
  //    sectionKey "manage") übernimmt das Ein-/Ausklappen. Der alte
  //    v47-Kopf (Icon/Untertitel) wird per CSS ausgeblendet.
  ...
  // Kein eigenes Ein-/Ausklappen mehr – die native Sektion steuert das.
}
