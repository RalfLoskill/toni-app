...
function toniRenderReflexion(task,el){
  ...
  const showScale = task.reflexion_scale !== false;
  const scaleLabel = task.reflexion_scale_label || 'Wie gut verstehst du das Thema?';
  ...
  const scaleHtml = showScale
    ? `<div style="margin-bottom:12px"><div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:6px">${toniEsc(scaleLabel)}</div><div style="display:flex;gap:6px" id="reflexion-stars">${[1,2,3,4,5].map(i=>`<button onclick="toniReflexionStar(${i})" style="font-size:22px;background:none;border:none;cursor:pointer;opacity:.35" data-star="${i}">⭐</button>`).join('')}</div></div>`
    : '';
  const helpersHtml = helpers.length
    ...
    : '';
  el.innerHTML=`<div style="font-size:14px;color:var(--color-text-primary);line-height:1.7;margin-bottom:12px">${toniEsc(prompt)}</div>${scaleHtml}${helpersHtml}`;
}
