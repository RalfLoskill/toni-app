...
/* ============================================================
   TONI – Lerninhalt V2 (sauberer Ansatz)
   Wrap von buildTaskFromVisibleForm + renderLerninhaltModal
   ============================================================ */
(function(){
  ...
  function isLerninhalt(type){
    const t = String(type||'').toLowerCase();
    return ['lerninhalt','info','erklärung','material'].includes(t);
  }
  ...
  // ── buildTaskFromVisibleForm wrappen ───────────────────────
  // Wir warten bis die Funktion verfügbar ist, dann wrappen wir sie einmalig
  function wrapBuildTask(){
    const orig = window.buildTaskFromVisibleForm;
    if(!orig || orig.__toniLerninhaltWrapped) return false;

    window.buildTaskFromVisibleForm = function(){
      const task = orig.apply(this, arguments);
      if(!task) return task;

      // Lerninhalt-Blocks anhängen falls vorhanden
      if(isLerninhalt(task.type)){
        const blocks = [
        ...
        if(blocks.length){
          task.lerninhalt_blocks = blocks;
          // Reset
          window.TONI_LERNINHALT_IMAGES = [];
          window.TONI_LERNINHALT_FILES = [];
          window.TONI_LERNINHALT_LINKS = [];
          const status = document.getElementById('lerninhalt-save-status');
          if(status) { status.innerHTML = `✅ ${blocks.length} Inhalt(e) gespeichert`; setTimeout(()=>{status.innerHTML='';},3000); }
          console.log('✅ TONI Lerninhalt: Blocks in Task eingebaut', task.title, blocks.length);
        }
      }
      return task;
    };
    window.buildTaskFromVisibleForm.__toniLerninhaltWrapped = true;
    console.log('✅ TONI buildTaskFromVisibleForm gepatcht');
    return true;
  }

  // Polling bis Funktion bereit ist
  let tries = 0;
  const poll = setInterval(() => {
    tries++;
    if(wrapBuildTask() || tries > 40) clearInterval(poll);
  }, 200);

  // ── Lerninhalt-Modal rendern (Schüler-Sicht) ───────────────
  function renderLerninhaltModal(task){
    ...
    const rawBlocks = task.lerninhalt_blocks;
    if(rawBlocks){
      const blocks = typeof rawBlocks==='string' ? JSON.parse(rawBlocks) : rawBlocks;
      blocks.forEach(b => {
    ...
    if(task.links) links = [...links, ...task.links];

    body.innerHTML = `
      <div style="max-width:100%;line-height:1.7">
        ${textHtml ? `<div style="font-size:14px;color:var(--color-text-primary);margin-bottom:12px">${textHtml}</div>` : ''}

        ${images.length ? `
          <div style="display:flex;flex-wrap:wrap;gap:8px;margin:10px 0">
            ${images.map(img=>`
              <img src="${esc(img.url)}" alt="${esc(img.alt||'Bild')}"
                style="max-width:100%;max-height:280px;border-radius:8px;border:0.5px solid var(--color-border-tertiary);object-fit:contain;cursor:pointer"
          ...
          <div style="margin:10px 0">
            ...
            ${files.map(f=>`
              <a href="${esc(f.url)}" target="_blank" download
                style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;margin-bottom:6px;text-decoration:none;color:var(--color-text-primary)">
                <span style="font-size:18px">📄</span>
                <span style="font-size:13px;flex:1">${esc(f.name||f.url.split('/').pop())}</span>
                <span style="font-size:11px;color:var(--color-text-tertiary)">↓ Download</span>
              </a>`).join('')}
          </div>` : ''}
          ...
          <div style="margin:10px 0">
            ...
            ${links.map(l=>`
              <a href="${esc(l.url)}" target="_blank" rel="noopener"
                style="display:flex;align-items:center;gap:8px;padding:8px 12px;background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:8px;margin-bottom:6px;text-decoration:none;color:#0C447C">
  ...
  function hookOpenTask(){
    ...
    if(!orig || orig.__toniLerninhaltV2) return false;
    ...
    window.openLearningTask = function(id){
      ...
      if(task && isLerninhalt(task.type)){
        ...
        if(typeof syncJourneyToDashboard==='function') setTimeout(syncJourneyToDashboard, 100);
        return;
      }
      return orig.apply(this, arguments);
    };
    window.openLearningTask.__toniLerninhaltV2 = true;
    window.openLearningTask.__toniV108Wrapped = true;
    console.log('✅ TONI openLearningTask gepatcht');
    return true;
  }
  ...
  const hookPoll = setInterval(() => {
    hookTries++;
    if(hookOpenTask() || hookTries > 40) clearInterval(hookPoll);
  }, 200);

  // ── Tutor: Builder-Extras ───────────────────────────────────
  function enhanceTaskBuilder(){
    const typeSelect = document.getElementById('task-type');
    if(!typeSelect || typeSelect.__toniLerninhaltV2) return;
    typeSelect.__toniLerninhaltV2 = true;

    const container = typeSelect.closest('form') || typeSelect.closest('.lr-modal') || typeSelect.parentElement?.parentElement;
    if(!container) return;
    ...
    extras.innerHTML = `
      <div style="background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:12px">
        <div style="font-size:12px;font-weight:500;color:#185FA5;margin-bottom:10px">📖 Lerninhalt-Medien</div>

        <div style="margin-bottom:10px">
          <label style="font-size:11px;color:var(--color-text-tertiary);display:block;margin-bottom:4px">Bild hinzufügen</label>
          <input type="file" id="lerninhalt-img-upload" accept="image/*" onchange="toniHandleImgUpload(this)" style="font-size:12px;width:100%">
          <div id="lerninhalt-img-preview" style="margin-top:6px"></div>
        </div>

        <div style="margin-bottom:10px">
          <label style="font-size:11px;color:var(--color-text-tertiary);display:block;margin-bottom:4px">Datei-Anhang (PDF, Word…)</label>
          <input type="file" id="lerninhalt-file-upload" accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx" onchange="toniHandleFileUpload(this)" style="font-size:12px;width:100%">
        ...
        <div style="margin-bottom:10px">
          ...
          <div style="display:flex;gap:6px;margin-bottom:4px">
            <input type="url" id="lerninhalt-link-url" placeholder="https://..." style="flex:2;padding:6px 8px;border:0.5px solid var(--color-border-secondary);border-radius:6px;font-size:12px;background:var(--color-background-primary);color:var(--color-text-primary)">
            <input type="text" id="lerninhalt-link-title" placeholder="Titel" style="flex:1;padding:6px 8px;border:0.5px solid var(--color-border-secondary);border-radius:6px;font-size:12px;background:var(--color-background-primary);color:var(--color-text-primary)">
            <button type="button" onclick="toniAddLink()" style="padding:6px 10px;background:#185FA5;color:#fff;border:none;border-radius:6px;font-size:12px;cursor:pointer">+</button>
          </div>
          <div id="lerninhalt-links-list"></div>
        </div>

        <div id="lerninhalt-save-status" style="font-size:12px;color:#27500A;text-align:center;min-height:18px"></div>
      </div>`;

    const descGroup = container.querySelector('#task-description')?.closest('.lr-form-group');
    if(descGroup) descGroup.after(extras);
  ...
  window.toniHandleImgUpload = async function(input){
    ...
    try {
      ...
      const res = await fetch(`${window.SUPABASE_URL}/storage/v1/object/learning-content/${path}`,{
        method:'POST', headers:{'apikey':window.SUPABASE_ANON_KEY,'Authorization':'Bearer '+token,'Content-Type':file.type,'x-upsert':'true'}, body:file
      });
      ...
      if(preview) preview.innerHTML = `<img src="${url}" style="max-height:80px;border-radius:6px;border:0.5px solid var(--color-border-tertiary)"><div style="font-size:11px;color:#27500A;margin-top:2px">✅ Hochgeladen</div>`;
    } catch(e){ if(preview) preview.innerHTML=`<span style="color:#A32D2D;font-size:11px">Fehler: ${esc(e.message)}</span>`; }
  };
  ...
  window.toniHandleFileUpload = async function(input){
    ...
    } catch(e){ if(list) list.innerHTML=`<span style="color:#A32D2D;font-size:11px">Fehler: ${esc(e.message)}</span>`; }
  };
  ...
  window.toniAddLink = function(){
    ...
    if(list) list.innerHTML = window.TONI_LERNINHALT_LINKS.map(l=>`<div style="font-size:11px;color:var(--color-text-secondary)">🔗 ${esc(l.title)}</div>`).join('');
  };

  // Observer für Builder
  const obs = new MutationObserver(() => {
    if(document.getElementById('task-type') && !document.getElementById('task-type').__toniLerninhaltV2){
      enhanceTaskBuilder();
      hookOpenTask();
      wrapBuildTask();
    }
  ...
  if(document.readyState!=='loading') {
    ...
    document.addEventListener('DOMContentLoaded', enhanceTaskBuilder);
  }
