...
(function(){
  ...
  window.handleLerninhaltImageUpload = async function(input){
    ...
    try {
      ...
      if(preview) preview.innerHTML = `<img src="${url}" style="max-height:60px;border-radius:4px;border:0.5px solid var(--color-border-tertiary)"> <span style="font-size:11px;color:#27500A">✅ Hochgeladen</span>`;
    } catch(e) {
  ...
  window.addLerninhaltLink = function(){
    ...
    if(list) list.innerHTML = window.TONI_LERNINHALT_LINKS.map((l,i)=>
      `<div style="font-size:11px;color:var(--color-text-secondary);padding:2px 0">${esc(l.title)} <span style="color:var(--color-text-tertiary)">${esc(l.url)}</span></div>`
    ).join('');
  };
  ...
  if(typeof origAddTask==='function' && !origAddTask.__toniLerninhaltWrapped){
    window.addTaskToStationBuilder = function(){
      ...
      if(isLerninhalt(type)){
        ...
        if(blocks.length){
          // Blocks temporär im window speichern damit origAddTask sie aufnehmen kann
          window.TONI_PENDING_LERNINHALT_BLOCKS = blocks;
        }
        // Reset
        window.TONI_LERNINHALT_IMAGES = [];
        window.TONI_LERNINHALT_FILES = [];
      ...
      if(window.TONI_PENDING_LERNINHALT_BLOCKS?.length){
        const tasks = window.TONI_CURRENT_STATION_TASKS;
        if(tasks?.length){
          tasks[tasks.length-1].lerninhalt_blocks = window.TONI_PENDING_LERNINHALT_BLOCKS;
        }
        window.TONI_PENDING_LERNINHALT_BLOCKS = null;
      }
    };
  ...
  if(document.readyState==='loading'){
    ...
    init();
    setTimeout(init, 500); // Fallback falls Builder später gerendert wird
  }
