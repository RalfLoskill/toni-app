...
(function(){
  ...
  function buildTaskFromVisibleForm(){
    ...
    // TONI Lerninhalt: Blocks (Bilder, Dateien, Links) anhängen
    if(['lerninhalt','info','erklärung','material'].includes(String(type).toLowerCase())){
      const blocks = [
      ...
      if(blocks.length){
        ...
        window.TONI_LERNINHALT_FILES = [];
        window.TONI_LERNINHALT_LINKS = [];
        console.log('✅ TONI: lerninhalt_blocks in Task eingebaut', blocks.length, 'Blöcke');
      }
    }
...
(function(){
  ...
  function renderLerninhaltModal(task){
    ...
    // Render
    // Neue Layout: Text links, Medien rechts
    const hasMedia = images.length || files.length || links.length;

    body.innerHTML = `
      <div style="display:flex;gap:16px;align-items:flex-start;flex-wrap:wrap">
        <div style="flex:1;min-width:200px">
          ${textHtml ? `<div style="font-size:14px;line-height:1.75;color:var(--color-text-primary);margin-bottom:14px">${textHtml}</div>` : ''}
          <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:12px">
            ...
            <button onclick="showSelectedTaskHint()" style="padding:10px 16px;background:none;border:0.5px solid var(--color-border-secondary);border-radius:8px;font-size:13px;cursor:pointer;color:var(--color-text-secondary)">Hinweis von TONI</button>
          </div>
        </div>
        ${hasMedia ? `
        <div style="width:250px;flex-shrink:0;background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:10px;padding:12px">
          ...
          ${images.map(img=>`<img src="${esc(img.url)}" alt="${esc(img.alt||'Bild')}" style="width:100%;border-radius:8px;margin-bottom:8px;cursor:pointer;border:0.5px solid var(--color-border-tertiary)" onclick="window.open('${esc(img.url)}','_blank')">`).join('')}
          ${files.map(f=>`<a href="${esc(f.url)}" target="_blank" download style="display:flex;align-items:center;gap:6px;padding:7px 9px;background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:7px;margin-bottom:6px;text-decoration:none;color:var(--color-text-primary)"><span>📄</span><span style="font-size:12px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(f.name||f.url.split('/').pop())}</span><span style="font-size:11px;color:var(--color-text-tertiary)">↓</span></a>`).join('')}
          ${links.map(l=>`<a href="${esc(l.url)}" target="_blank" rel="noopener" style="display:flex;align-items:center;gap:6px;padding:7px 9px;background:#E6F1FB;border:0.5px solid #B5D4F4;border-radius:7px;margin-bottom:6px;text-decoration:none;color:#0C447C"><span>${l.youtube?'▶️':'🔗'}</span><span style="font-size:12px;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(l.title||l.url)}</span><span style="font-size:11px;opacity:.7">→</span></a>`).join('')}
        </div>` : ''}
      </div>
    ...
    extras.innerHTML = `
      <div style="background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:10px">
        ...
        <div style="margin-top:10px">
          <button type="button" onclick="toniLerninhaltSaveBlocks()"
            style="width:100%;padding:8px;background:#185FA5;color:#fff;border:none;border-radius:8px;font-size:13px;font-weight:500;cursor:pointer">
            ✅ Inhalte übernehmen
          </button>
          <div id="lerninhalt-save-status" style="font-size:11px;color:#27500A;margin-top:4px;text-align:center"></div>
        </div>
      </div>`;
  ...
  // "Inhalte übernehmen" Button – speichert Blocks in window für nächsten Task
  window.toniLerninhaltSaveBlocks = function(){
    const blocks = [
      ...(window.TONI_LERNINHALT_IMAGES||[]),
      ...(window.TONI_LERNINHALT_FILES||[]),
      ...(window.TONI_LERNINHALT_LINKS||[]),
    ...
    if(!blocks.length){
      ...
      if(status) status.textContent = '⚠️ Keine Inhalte zum Übernehmen.';
      return;
    }
    window.TONI_PENDING_LERNINHALT_BLOCKS_V2 = [...blocks];
    const status = document.getElementById('lerninhalt-save-status');
    if(status) status.innerHTML = `✅ ${blocks.length} Inhalt(e) bereit. Jetzt "Aufgabe hinzufügen" klicken.`;
    console.log('✅ TONI Lerninhalt: Blocks bereit für nächsten Task', blocks.length);
  };
...
/* ============================================================
   TONI – Lerninhalt Blocks Fix V2
   Injiziert vorgemerkten Blocks nach addTask
   ============================================================ */
(function(){
  // Nach click mit mehreren Delays versuchen
  document.addEventListener('click', function(event){
    const btn = event.target.closest?.('button');
    if(!btn) return;
    const text = (btn.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();

    if(!text.includes('aufgabe hinzufügen') && !text.includes('aufgabe zur station')) return;

    const blocks = window.TONI_PENDING_LERNINHALT_BLOCKS_V2;
    if(!blocks?.length) return;
    ...
    [100, 250, 500, 800].forEach(delay => {
      setTimeout(() => {
        ...
        if(last && !last.lerninhalt_blocks?.length){
          last.lerninhalt_blocks = [...blocks];
          window.TONI_PENDING_LERNINHALT_BLOCKS_V2 = null;
          window.TONI_LERNINHALT_IMAGES = [];
          window.TONI_LERNINHALT_FILES = [];
          window.TONI_LERNINHALT_LINKS = [];
          const status = document.getElementById('lerninhalt-save-status');
          if(status) status.innerHTML = '';
          console.log(`✅ TONI Blocks injiziert (${delay}ms):`, last.title, blocks.length, 'Blöcke');
        }
      }, delay);
    });
  }, false);
