...
function parseJourneyQrPayloadV20(raw){const text=String(raw||"").trim();if(!text)throw new Error("Kein QR-Code erkannt.");if(text.startsWith("TONI-JOURNEY:")){const rest=text.replace("TONI-JOURNEY:","").trim();const id=rest.split(":")[0].trim();if(!id)throw new Error("Der QR-Code enthält keine Lernreise-ID.");return id;}try{const url=new URL(text);const id=url.searchParams.get("journey")||url.searchParams.get("journey_id")||url.searchParams.get("lernreise");if(id)return id.trim();}catch{}if(toniV20UuidLike(text)||text.length>=12)return text;throw new Error("Der QR-Code ist kein gültiger TONI-Lernreise-Code.");}
...
function downloadJourneyQrCode(){
  ...
  const payload=journeyQrPayloadV20(id,grp);
  const url=qrImageUrlV20(payload,640);
  const a=document.createElement("a");
  const safeTitle=String(window.TONI_QR_CURRENT_JOURNEY_TITLE||"lernreise").toLowerCase().replace(/[^a-z0-9äöüß]+/gi,"-").replace(/^-|-$/g,"");
  a.href=url;a.download=`toni-qr-${safeTitle||id}.png`;a.target="_blank";document.body.appendChild(a);a.click();a.remove();
}
