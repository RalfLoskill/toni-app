...
function openLearningTask(id){
  ...
  toniSetStartButtonState(task.status==='in_progress'||task.status==='done');
...
function startSelectedLearningTask(){const f=findTask(STATE.selectedTaskId);if(!f)return;f.task.status='in_progress';toniSetStartButtonState(true);syncJourneyToDashboard();renderLearningJourneyModal();showXPToast(5);}

// Starten-Button optisch umschalten: true = grün (gestartet), false = blau (zurückgesetzt)
function toniSetStartButtonState(started){
  const btn=document.querySelector('#lr-task-modal .lr-iconbtn-start');
  if(!btn) return;
...
(function(){
  ...
  function toniSlimTaskRow(task, opts){
    ...
    const styleAttr=`style="border-left:4px solid ${col.bar};background:${col.bg}"`;
    if(clickable){
