...
(function(){
  ...
  window.toniSlimLogin = function(){
    ...
    // SuperAdmin-Sonderfall: Der SuperAdmin meldet sich nicht über Supabase an,
    // sondern über das feste SuperAdmin-Passwort (V77). Der automatische
    // Zwei-Schritt-Ablauf mit 350ms-Timer passt dafür nicht, weil das Passwort
    // beim ersten Klick noch nicht eingegeben ist. Daher hier eigener Ablauf:
    const sa = window.toniV77SuperAdminLogin;
    const isSuperAdmin = sa && typeof sa.isSuperAdminName === "function"
      ? sa.isSuperAdminName(email)
      : (email === "SuperAdmin");
    if(isSuperAdmin){
      const pwArea = document.getElementById("auth-password-area");
      const pwVisible = pwArea && pwArea.classList.contains("visible");
      // Erster Schritt: Passwortfeld noch nicht sichtbar -> einblenden, dann warten.
      if(!pwVisible || !pw){
        try{ if(typeof sa.showSuperAdminPassword === "function") sa.showSuperAdminPassword(); }catch(e){ console.warn("SuperAdmin:", e); }
        return;
      }
      // Zweiter Schritt: Passwort liegt vor -> SuperAdmin direkt anmelden.
      // Bevorzugt der direkte Aufruf von trySuperAdminLogin (umgeht Wrapper-Unsicherheiten);
      // fällt sonst auf window.signInWithPassword zurück (greift via V77-Wrapper).
      try{
        if(sa && typeof sa.trySuperAdminLogin === "function"){
          sa.trySuperAdminLogin();
        } else if(typeof window.signInWithPassword === "function"){
          window.signInWithPassword();
        }
      }catch(e){ console.warn("SuperAdmin-Login:", e); }
      return;
    }
