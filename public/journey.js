...
/* ============================================================
   TONI – Lerninhalt Blocks Fix V3 (Proxy)
   ============================================================ */
(function(){

  function installProxy(){
    const arr = window.TONI_CURRENT_STATION_TASKS;
    if(!Array.isArray(arr) || arr.__toniProxied) return;

    const proxy = new Proxy(arr, {
      get(target, prop){
        if(prop === '__toniProxied') return true;
        if(prop === Symbol.toStringTag) return 'Array';
        if(prop === Symbol.iterator) return target[Symbol.iterator].bind(target);
        if(prop === 'push'){
          return function(){
            const result = Array.prototype.push.apply(target, arguments);
            ...
            if(blocks?.length){
              ...
              if(last){
                ...
                window.TONI_LERNINHALT_LINKS = [];
                const status = document.getElementById('lerninhalt-save-status');
                if(status) { status.innerHTML = `✅ Inhalte in "${last.title||'Aufgabe'}" gespeichert`; setTimeout(()=>{status.innerHTML='';},3000); }
                console.log('✅ TONI Proxy push: Blocks injiziert in', last.title, blocks.length, 'Blöcke');
              }
            }
            return result;
          };
    ...
    // Array.isArray täuschen damit ensureArrayState den Proxy nicht ersetzt
    const origIsArray = Array.isArray;
    if(!Array.isArray.__toniPatched){
      Array.isArray = function(val){
        if(val && val.__toniProxied) return true;
        return origIsArray(val);
      };
      Array.isArray.__toniPatched = true;
    }
    window.TONI_CURRENT_STATION_TASKS = proxy;
    console.log('✅ TONI Proxy auf TONI_CURRENT_STATION_TASKS installiert');
  }

  // Installieren sobald Array bereit ist
  function tryInstall(){
    if(window.TONI_CURRENT_STATION_TASKS && !window.TONI_CURRENT_STATION_TASKS.__toniProxied){
      installProxy();
    }
  }

  [100, 500, 1000, 2000, 3000].forEach(d => setTimeout(tryInstall, d));

  // Nach clearStationBuilder neu installieren
  const obs = new MutationObserver(() => tryInstall());
  document.addEventListener('DOMContentLoaded', () => {
    obs.observe(document.body, {childList:true, subtree:false});
  });

  // clearStationBuilder hooken
  setTimeout(() => {
    const origClear = window.clearStationBuilder;
    if(typeof origClear==='function' && !origClear.__toniProxyWrapped){
      window.clearStationBuilder = function(){
        const r = origClear.apply(this, arguments);
        setTimeout(tryInstall, 50);
        return r;
      };
      window.clearStationBuilder.__toniProxyWrapped = true;
    }
  }, 1000);
