...
window.TONI_JOURNEY_BUILD = "v169-reflexion-wortskala";
...
function toniRenderReflexion(task,el){
  ...
  // V169: Pro Skala wählbare Darstellung. sc.scale_type: "words" (Wortskala,
  // 4 Stufen) oder "stars" (Sterne 1-5, Standard/Fallback). Gespeichert wird in
  // beiden Fällen ein Zahlenwert in _reflexionStars[scaleIdx].
  ...
  const scalesHtml = scales.map((sc,si)=>{
    ...
    if(sc.scale_type === "words"){
      control = `<div style="display:flex;gap:6px;flex-wrap:wrap" class="reflexion-words" data-scale="${si}">` +
        TONI_WORT_STUFEN.map((w,i)=>`<button onclick="toniReflexionWord(${si},${i+1})" data-step="${i+1}" style="flex:1;min-width:78px;padding:8px 10px;background:var(--color-background-primary);border:0.5px solid var(--color-border-secondary);border-radius:8px;font-size:13px;color:var(--color-text-primary);cursor:pointer">${toniEsc(w)}</button>`).join('') +
        `</div>`;
    } else {
      control = `<div style="display:flex;gap:6px" class="reflexion-stars" data-scale="${si}">` +
        [1,2,3,4,5].map(i=>`<button onclick="toniReflexionStar(${si},${i})" style="font-size:22px;background:none;border:none;cursor:pointer;opacity:.35" data-star="${i}">⭐</button>`).join('') +
        `</div>`;
    }
...
// V169: Wortskala – markiert die gewählte Stufe, speichert denselben Zahlenwert.
