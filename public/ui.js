...
/* =========================================================
   TONI V83-DIAG – Ladevorhang-Diagnose. Aktivierung OHNE URL-Hash
   (vermeidet 404 durch Routing). In der Browser-Konsole eingeben:
       toniDiagOn()    → Diagnose einschalten (bleibt über Reloads aktiv)
       toniDiagOff()   → ausschalten und Protokoll löschen
   Protokolliert reload-übergreifend in sessionStorage.
   ========================================================= */
(function(){
  const KEY = "TONI_LOADDIAG";
  const LOGKEY = "TONI_LOADDIAG_LOG";

  // sessionStorage kann in manchen Kontexten gesperrt sein – defensiv kapseln.
  function ssGet(k){ try { return sessionStorage.getItem(k); } catch(_){ return null; } }
  function ssSet(k,v){ try { sessionStorage.setItem(k,v); } catch(_){} }
  function ssDel(k){ try { sessionStorage.removeItem(k); } catch(_){} }

  // Globale Schalter – in der Konsole aufrufbar
  window.toniDiagOn = function(){
    ssSet(KEY, "1");
    ssDel(LOGKEY);
    location.reload();
  };
  window.toniDiagOff = function(){
    ssDel(KEY);
    ssDel(LOGKEY);
    location.reload();
  };
  ...
  // Protokoll aus sessionStorage laden (übersteht Reload)
  let log = [];
  try { log = JSON.parse(ssGet(LOGKEY) || "[]"); } catch(_) { log = []; }

  const t0 = Date.now();
  function stamp(){ return ("+" + (Date.now()-t0) + "ms").padStart(8); }
  function add(line){
    log.push(stamp() + "  " + line);
    ssSet(LOGKEY, JSON.stringify(log.slice(-200)));
    render();
  }

  // Markieren, ob das hier ein frischer Seitenload/Reload ist
  add("──── Seitenload (" + new Date().toLocaleTimeString() + ", " + (performance?.navigation?.type === 1 ? "RELOAD" : "normal") + ") ────");

  // toniReady instrumentieren
  function wrapReady(){
    if(!window.toniReady){ setTimeout(wrapReady, 50); return; }
    if(window.toniReady.__diagWrapped) return;
    const orig = window.toniReady;
    window.toniReady = {
      __diagWrapped: true,
      begin(){ add("toniReady.begin() AUFGERUFEN"); return orig.begin.apply(orig, arguments); },
      done(k){ add("toniReady.done('" + k + "')"); return orig.done.apply(orig, arguments); },
      status(){ return orig.status.apply(orig, arguments); }
    };
    add("toniReady instrumentiert");
  }
  wrapReady();

  // toniV73ShowLoader instrumentieren (mit Wiederholung, falls noch nicht definiert)
  function wrapShow(){
    if(typeof window.toniV73ShowLoader !== "function"){ setTimeout(wrapShow, 50); return; }
    if(window.toniV73ShowLoader.__diagWrapped) return;
    const origShow = window.toniV73ShowLoader;
    window.toniV73ShowLoader = function(dur, reason){
      add("toniV73ShowLoader(reason='" + reason + "')");
      return origShow.apply(this, arguments);
    };
    window.toniV73ShowLoader.__diagWrapped = true;
    add("toniV73ShowLoader instrumentiert");
  }
  wrapShow();

  // Auth-Events mitschneiden (der echte Login-Pfad!)
  function hookAuth(){
    try {
      const client = (typeof getSupabaseClient === "function") ? getSupabaseClient() : null;
      if(!client || !client.auth || !client.auth.onAuthStateChange){ setTimeout(hookAuth, 200); return; }
      if(window.__diagAuthHooked) return;
      window.__diagAuthHooked = true;
      client.auth.onAuthStateChange((event) => add("AUTH-EVENT: " + event));
      add("Auth-Events werden mitgeschnitten");
    } catch(e){ setTimeout(hookAuth, 200); }
  }
  hookAuth();

  // Welcher Loader ist sichtbar?
  setInterval(() => {
    const v69 = document.getElementById("toni-loading-screen-v69");
    const v73 = document.getElementById("toni-loading-screen-v73");
    const vis = el => el ? (el.classList.contains("hidden") ? "hidden" : "SICHTBAR") : "—";
    const cur = "v69:" + vis(v69) + " v73:" + vis(v73);
    if(cur !== window.__lastLoaderState){
      window.__lastLoaderState = cur;
      add("Loader-Status: " + cur);
    }
  }, 200);
  ...
  function render(){
    ...
    if(!box){
      ...
      box.style.cssText = "position:fixed;left:0;right:0;bottom:0;z-index:999999;background:#0f172a;color:#7CFC9C;font:11px/1.45 monospace;padding:10px;white-space:pre-wrap;max-height:55vh;overflow:auto;border-top:3px solid #22c55e";
      document.body.appendChild(box);
    }
    box.textContent = "=== LADEVORHANG-DIAGNOSE (aktiv bis #loaddiag-off) ===\n" + log.join("\n");
