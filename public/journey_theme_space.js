...
(function () {
  ...
  const MARGIN_X = 300;        // Auslenkung der Bahn nach links/rechts (breiter nutzen)
  const STEP_GAP = 165;        // vertikaler Abstand zwischen zwei Stationen (kompakter)
  const PAD_TOP = 150;         // Platz oben für den Zielstern
  const PAD_BOTTOM = 140;      // Platz unten für den Start
  ...
  function computeLayout(n) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Phase 1.15 sorgt für ein ausgeprägteres Links-Rechts über die Breite.
...
/* --- Layout-Anpassung NUR wenn das Weltall-Theme aktiv ist --- */
/* Stationen breit oben, Aufgaben darunter (statt schmal links | rechts). */
.toni-theme-active-space .lr-top-split{
  grid-template-columns:1fr !important;
  grid-template-areas:"stations" "right" !important;
}
/* Der Stationsbereich braucht keinen hellen Kasten – das Theme bringt
   seinen eigenen dunklen Weltraum-Hintergrund mit. */
.toni-theme-active-space .lr-top-split .lr-stations{
  background:transparent !important;
  padding:0 !important;
}
/* Etwas Luft zwischen Sternenkarte und Aufgabenbereich. */
.toni-theme-active-space .lr-right-col{
  margin-top:18px;
}`;
  ...
  console.info("[TONI-Theme:space] Weltall-Theme registriert (theme-space-v2-wide).");
})();
