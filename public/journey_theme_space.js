...
.toni-space__starspin{position:absolute;left:50%;top:50%;width:160%;height:160%;
  transform:translate(-50%,-50%);z-index:0;pointer-events:none;
  animation:toniSpaceRotate 180s linear infinite;transform-origin:center;}
.toni-space__starspin i{position:absolute;background:#fff;border-radius:50%;opacity:.55;}
...
`;
  ...
  function renderStations(journey) {
    ...
    let starsHTML = "";
    const starCount = Math.min(220, 110 + n * 9);
    ...
    // Zweite Sternenebene, die langsam rotiert (Tiefenwirkung).
    let spinStarsHTML = "";
    const spinCount = Math.min(140, 70 + n * 6);
  ...
  console.info("[TONI-Theme:space] Weltall-Theme registriert (theme-space-v5-cosmos).");
})();
