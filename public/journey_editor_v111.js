...
(function(){
  ...
  function lerninhaltEditorHtml(t,si,ti){
    ...
    return `<div class="v111-rxcard v111-lerncard">
      ...
      <div class="v111-lerntoolbar">
        <button type="button" class="v111-lernbtn" title="Fett (Text markieren, dann klicken)"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'bold')"><b>F</b></button>
        <button type="button" class="v111-lernbtn" title="Kursiv (Text markieren, dann klicken)"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'italic')"><i>K</i></button>
        <button type="button" class="v111-lernbtn" title="Hochstellen, z. B. x²"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'sup')">x<sup>2</sup></button>
        <button type="button" class="v111-lernbtn" title="Tiefstellen, z. B. H₂O"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'sub')">x<sub>2</sub></button>
        <button type="button" class="v111-lernbtn" title="Aufzählung"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'list')">• Liste</button>
        <button type="button" class="v111-lernbtn" title="Überschrift (aktuelle Zeile)"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'heading')">Überschrift</button>
      </div>
      <div id="v111-content-${si}-${ti}" class="v111-lernedit" contenteditable="true"
        data-ph="Hier den eigentlichen Lerntext eingeben…"
        oninput="TONI_EDITOR_V111_UI.editContentHtml(${si},${ti},this.innerHTML)">${html}</div>
      <div style="font-size:11px;color:var(--v111-t3);margin-top:6px">Text markieren und Button klicken. „Überschrift“ und „Liste“ wirken auf die aktuelle Zeile. Die Lernenden sehen den Text formatiert.</div>
    </div>`;
  }
  ...
  const UI = {
    ...
    editContentHtml(si,ti,html){ const t=stations()[si]?.tasks[ti]; if(t){ t.content=html;
      if(typeof syncJourneyBuilderToLegacyTextareaV17==="function")syncJourneyBuilderToLegacyTextareaV17(); } },
    // V112: Mini-Toolbar-Befehl (Fett/Absatz) auf dem aktiven Lernstoff-Feld.
    contentCmd(si,ti,cmd){
      const ed=document.getElementById("v111-content-"+si+"-"+ti);
      if(!ed) return;
      ed.focus();
      try{
        switch(cmd){
          ...
          case "heading":   document.execCommand("formatBlock", false, "h4"); break;
        }
      }catch(e){}
      const t=stations()[si]?.tasks[ti];
      if(t){ t.content=ed.innerHTML; if(typeof syncJourneyBuilderToLegacyTextareaV17==="function")syncJourneyBuilderToLegacyTextareaV17(); }
    },
  ...
  function injectStyles(){
    ...
    css.textContent = `
      ...
      ${S} .v111-lernedit h4{font-size:14px!important;font-weight:600!important;margin:10px 0 4px!important;color:var(--v111-t1)!important}
      ${S} .v111-lernedit ul{margin:6px 0 8px!important;padding-left:20px!important;list-style:disc!important}
