...
(function(){
  ...
  function lerninhaltEditorHtml(t,si,ti){
    ...
    return `<div class="v111-rxcard v111-lerncard">
      ...
      <div class="v111-lerntoolbar">
        <button type="button" class="v111-lernbtn" title="Fett (markieren und klicken)"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'bold')"><b>F</b></button>
        <button type="button" class="v111-lernbtn" title="Neuer Absatz"
          onmousedown="event.preventDefault()" onclick="TONI_EDITOR_V111_UI.contentCmd(${si},${ti},'paragraph')">¶ Absatz</button>
      </div>
      ...
      <div style="font-size:11px;color:var(--v111-t3);margin-top:6px">Text markieren und „F“ klicken für Fettdruck. „Absatz“ trennt Abschnitte. Die Lernenden sehen den Text formatiert.</div>
    </div>`;
  }
  ...
  const UI = {
    ...
    contentCmd(si,ti,cmd){
      ...
      if(!ed) return;
      ed.focus();
      try{ document.execCommand(cmd==="bold"?"bold":"formatBlock", false, cmd==="bold"?null:"p"); }catch(e){}
      const t=stations()[si]?.tasks[ti];
