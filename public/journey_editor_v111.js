...
(function(){
  ...
  // Reflexions-Editor: Leitfrage, Sterne-Skala (an/aus + Label), Hilfsfragen
  function reflexionEditorHtml(t,si,ti){
    const scaleOn = t.reflexion_scale !== false;
    ...
    return `<div class="v111-reflex">
      ...
      <div class="v111-rxcard">
        <label class="v111-rxrow">
          <span>Selbsteinschätzung (Sterne 1–5)</span>
          <input type="checkbox" ${scaleOn?'checked':''}
            onchange="TONI_EDITOR_V111_UI.reflexionToggleScale(${si},${ti},this.checked)">
        </label>
        ${scaleOn?`<input class="v111-inp" style="margin-top:8px" value="${E.esc(t.reflexion_scale_label||"Wie gut verstehst du das Thema?")}"
          placeholder="Frage zur Skala"
          oninput="TONI_EDITOR_V111_UI.editTask(${si},${ti},'reflexion_scale_label',this.value)">`:""}
      </div>
  ...
  const UI = {
    ...
    // Reflexions-Editor
    reflexionToggleScale(si,ti,on){ const t=stations()[si]?.tasks[ti]; if(t){ t.reflexion_scale=!!on; commit(); } },
    ...
    setVideo(si,ti,raw){
      ...
      // Nur neu rendern, wenn sich der Gültigkeitszustand ändert (Vorschau ein/aus) – sonst Fokus halten
