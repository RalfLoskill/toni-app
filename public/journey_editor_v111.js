...
(function(){
  ...
  // ---- Einhängen: Editor-Host direkt vor das versteckte Strukturfeld setzen ----
  function ensureHost(){
    let host = document.getElementById(HOST_ID);
    if(host) return host;
    const legacy = field(LEGACY_FIELD_ID);
    if(!legacy || !legacy.parentElement) return null;
    // Legacy-Feld verstecken (bleibt als Datenträger erhalten)
    const group = legacy.closest(".lr-form-group") || legacy.parentElement;
    group.style.display = "none";
    host = document.createElement("div");
    host.id = HOST_ID;
    group.parentElement.insertBefore(host, group);
    return host;
  }
