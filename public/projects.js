...
(function waitForAuth() {
  ...
  // 2. Supabase Auth State Change (feuert nach jedem Login/Logout)
  ...
  // 3. applyAuthProfile hooken (wird nach Profil-Laden aufgerufen)
  ...
  // 4. Polling als letzter Fallback
  let tries = 0;
  const poll = setInterval(() => {
    tries++;
    if (!hookSupabase()) {} // retry hooking supabase
    tryLoad();
