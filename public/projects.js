...
function renderProjectsDashboard() {
  ...
  wrap.innerHTML = projects.map(p => {
    ...
    const extra = (p.member_count||0) > 5 ? `<span style="font-size:11px;color:var(--color-text-tertiary);margin-left:2px">+${p.member_count-5}</span>` : '';
    const avatars = members.map(m => avatarHtml(m, 24)).join('');

    const badges = [
      p.is_official ? `<span style="font-size:10px;background:#E1F5EE;color:#085041;padding:1px 6px;border-radius:10px">Offiziell</span>` : '',
      p.has_blocker ? `<span style="font-size:10px;background:#FAEEDA;color:#854F0B;padding:1px 6px;border-radius:10px">⚠ Blocker</span>` : '',
      p.type==='group' ? `<span style="font-size:10px;background:#E6F1FB;color:#185FA5;padding:1px 6px;border-radius:10px">Gruppe</span>` : '',
    ].filter(Boolean).join(' ');

    const deadlineStr = p.deadline ? formatDate(p.deadline) : '';
    const overdue = p.deadline && isOverdue(p.deadline) && p.status !== 'completed';
    const deadlineHtml = deadlineStr
      ? `<span style="font-size:11px;color:${overdue ? '#A32D2D' : 'var(--color-text-tertiary)'}">📅 ${deadlineStr}${overdue?' · überfällig':''}</span>`
      : '';

    return `<div onclick="openProjectModal('${p.id}')" style="cursor:pointer;padding:10px 0;border-bottom:0.5px solid var(--color-border-tertiary)">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:3px">
        <div style="font-size:14px;font-weight:500;color:var(--color-text-primary);line-height:1.3">${escapeHtml(p.title)}</div>
        <div style="font-size:13px;font-weight:500;color:${barColor};white-space:nowrap;flex-shrink:0">${pct}%</div>
      </div>
      ${p.description ? `<div style="font-size:12px;color:var(--color-text-secondary);margin-bottom:4px;line-height:1.4">${escapeHtml(p.description)}</div>` : ''}
      <div style="height:3px;background:var(--color-border-tertiary);border-radius:2px;margin:5px 0">
        <div style="height:3px;width:${pct}%;background:${barColor};border-radius:2px;transition:width .3s"></div>
      </div>
  ...
  const items = wrap.querySelectorAll('[onclick]');
...
function renderTaskCard(task, col) {
  ...
  const nextColor = { todo: '#185FA5', in_progress: '#854F0B', review: '#3B6D11', done: '#444441' };

  const dueColor = task.due_date && isOverdue(task.due_date) && col !== 'done' ? '#A32D2D' : 'var(--color-text-tertiary)';
  const dueHtml = task.due_date ? `<div style="font-size:11px;color:${dueColor};margin-top:3px">${isOverdue(task.due_date)&&col!=='done'?'⚠ ':''}${formatDate(task.due_date)}</div>` : '';
  const blockerHtml = task.blocker ? `<div style="font-size:11px;color:#993C1D;background:#FAECE7;padding:3px 7px;border-radius:6px;margin-top:5px;line-height:1.3">⚠️ ${escapeHtml(task.blocker)}</div>` : '';
  const doneStyle = col === 'done' ? 'text-decoration:line-through;color:var(--color-text-tertiary)' : '';

  const assigneeHtml = profile
    ? `<div style="display:flex;align-items:center;gap:4px;margin-top:4px">${avatarHtml(profile,18)}<span style="font-size:11px;color:var(--color-text-secondary)">${escapeHtml(profile.first_name||profile.display_name||'')}</span></div>`
    : '';

  return `<div style="background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;padding:9px 10px;margin-bottom:6px;cursor:pointer;transition:border-color .15s"
    ...
    <div style="font-size:13px;font-weight:500;${doneStyle};line-height:1.3">${escapeHtml(task.title)}</div>
    ${assigneeHtml}${dueHtml}${blockerHtml}
    <button onclick="event.stopPropagation();moveProjectTask('${task.id}','${nextCol[col]}')"
      style="margin-top:7px;font-size:11px;padding:3px 9px;border:0.5px solid ${nextColor[col]};border-radius:10px;background:none;cursor:pointer;color:${nextColor[col]};font-weight:500">
      ${nextLabel[col]} →
    </button>
  </div>`;
}
