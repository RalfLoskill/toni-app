...
async function loadProjects() {
  try {
    ...
    // Bei Fehler keine veraltete Liste zeigen.
    window.TONI_PROJECTS = [];
    renderProjectsDashboard();
  }
}
...
function renderTaskCard(task, col) {
  ...
  // Jede Kachel bekommt immer eine Farbe: bevorzugt die des Inhabers,
  // sonst eine stabile Farbe anhand der Aufgaben-ID (kein Weiss mehr).
  const pal = getMemberPalette(profile ? profile.id : (task.id || ''));
...
async function openTaskDetail(taskId) {
  try {
    ...
    const pal = getMemberPalette(ownerProfile ? ownerProfile.id : (task.id || ''));
