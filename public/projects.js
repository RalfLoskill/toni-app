...
function renderTaskCard(task, col) {
  ...
  const dueColor = task.due_date && isOverdue(task.due_date) && col !== 'done' ? '#dc2626' : '#64748b';
  const dueHtml = task.due_date
    ? `<div style="font-size:11px;color:${dueColor};margin-top:3px">${isOverdue(task.due_date)&&col!=='done'?'⚠ ':''}${formatDate(task.due_date)}</div>`
    : '';
  ...
  const assigneeHtml = profile
    ? `<div style="display:flex;align-items:center;gap:6px;margin-top:5px">${avatarHtml(profile,22)}<div style="display:flex;flex-direction:column;line-height:1.15"><span style="font-size:11px;font-weight:600;color:#334155">${escapeHtml(memberPrimaryName(profile))}</span><span style="font-size:10px;color:#94a3b8">${escapeHtml(memberSubLabel(profile))}</span></div></div>`
    : '';
  ...
  const fwdBtn = `<button ${fwdEnabled ? `onclick="event.stopPropagation();moveProjectTask('${task.id}','${nextCol}')"` : 'disabled'}
      style="flex:1;font-size:11px;padding:4px 9px;border:1px solid #e2e8f0;border-radius:10px;font-weight:600;
      background:${fwdEnabled ? '#f8fafc' : 'transparent'};color:#475569;
  ...
  const resetBtn = `<button ${resetEnabled ? `onclick="event.stopPropagation();moveProjectTask('${task.id}','${prevCol}')"` : 'disabled'}
      title="Zurücksetzen"
      style="font-size:11px;padding:4px 9px;border:1px solid #e2e8f0;border-radius:10px;font-weight:600;
  ...
  // Variante B: weiße Karte, farbiger linker Rand zeigt die Inhaberfarbe.
  const cardBorder = isBlocked ? '1.5px solid #D8472B' : '1px solid var(--color-border-tertiary, #e2e8f0)';
  const leftAccent = isBlocked ? '#D8472B' : pal.bar;
  ...
  return `<div style="position:relative;background:#ffffff;border:${cardBorder};border-left:4px solid ${leftAccent};border-radius:8px;padding:9px 10px 9px 11px;margin-bottom:6px;cursor:pointer;box-shadow:0 1px 4px rgba(15,23,42,.05);transition:box-shadow .15s, transform .15s"
    onclick="openTaskDetail('${task.id}')"
    onmouseover="this.style.boxShadow='0 3px 10px rgba(15,23,42,.12)';this.style.transform='translateY(-1px)'" onmouseout="this.style.boxShadow='0 1px 4px rgba(15,23,42,.05)';this.style.transform='translateY(0)'">
    ${blockerBadge}
    <div style="font-size:13px;font-weight:600;color:#1e293b;line-height:1.3;${doneStyle};${isBlocked?'padding-right:18px':''}">${escapeHtml(task.title)}</div>
