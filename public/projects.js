...
function toniTaskFileIcon(type, name){
  ...
  if(/image\//.test(t) || /\.(png|jpe?g|gif|webp|heic)$/.test(t)) return { icon:"🖼️", bg:"#E6F1FB", fg:"#185FA5" };
  if(/pdf/.test(t)) return { icon:"📄", bg:"#FCEBEB", fg:"#A32D2D" };
  if(/word|\.docx?$/.test(t)) return { icon:"📝", bg:"#E6F1FB", fg:"#0C447C" };
  if(/excel|spreadsheet|\.xlsx?$/.test(t)) return { icon:"📊", bg:"#EAF3DE", fg:"#3B6D11" };
  return { icon:"📎", bg:"var(--color-background-secondary)", fg:"var(--color-text-secondary)" };
}
...
function renderTaskFiles(){
  ...
  if(!files.length){
    ...
    listEl.innerHTML = files.map(f=>{
      ...
      return `<div style="display:flex;align-items:center;gap:10px;background:var(--color-background-primary);border:0.5px solid var(--color-border-tertiary);border-radius:var(--border-radius-md);padding:9px 11px">
        <span style="width:30px;height:30px;flex-shrink:0;border-radius:6px;background:${ic.bg};display:flex;align-items:center;justify-content:center;font-size:16px">${ic.icon}</span>
        <div style="flex:1;min-width:0">
