...
function renderProjectsDashboard() {
  ...
  wrap.innerHTML = projects.map(p => {
    ...
    // Mitglieder-Avatare: Inhaber (created_by) zuerst und markiert, dann weitere.
    // So viele wie in die Kachelbreite passen (Rest als "+N").
    ...
    const sortedMembers = allMembers.slice().sort((a, b) => {
      ...
      return ao - bo; // Inhaber nach vorne
    });
    const MAX_AVATARS = 6;
    ...
    const avatarsHtml = shown.map(m => {
      ...
      // Inhaber: goldener Ring + Krönchen oben rechts.
      const ownerRing = isOwner ? "box-shadow:0 0 0 2px #f5b301;border-radius:50%;" : "";
      const crown = isOwner
        ? `<span title="Projektinhaber" style="position:absolute;top:-7px;right:-4px;font-size:12px;line-height:1;filter:drop-shadow(0 1px 1px rgba(0,0,0,.35))">👑</span>`
        : "";
      return `<div style="position:relative;${ownerRing}display:inline-flex">${avatarHtml(m, 30)}${crown}</div>`;
    }).join('');
    const restHtml = restCount > 0
      ? `<div style="width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,.85);color:#334155;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:800;border:2px solid #fff;flex-shrink:0">+${restCount}</div>`
      : '';
    const membersRow = (shown.length || restCount)
      ? `<div class="journey-tile-proj-members" style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:10px">${avatarsHtml}${restHtml}</div>`
      : '';

    // Projekt-Kachel im Lernreise-Kacheldesign (journey-tile-cover-v112):
    // gleicher Aufbau, gleiche Abmessungen wie die Lernreise-Kacheln.
    return `<div data-project-id="${p.id}" onclick="openProjectModal('${p.id}')"
      class="journey-tile journey-tile-cover-v112 has-cover journey-tile-clickable" style="background:${g.grad}" role="button" tabindex="0" title="Projekt öffnen">
      <div class="journey-tile-cover-body-v112">
        <div class="journey-tile-title journey-tile-cover-title-v112">${escapeHtml(p.title)}</div>
        <div class="journey-tile-cover-desc-v112">${escapeHtml(p.description || '–')}</div>
        <div class="journey-tile-chips journey-tile-cover-chips-v112">
          <span class="journey-tile-chip chip-tasks">${p.task_done||0}/${p.task_total||0} Aufgabe(n)</span>
          <span class="journey-tile-chip chip-stations">${memberCount} Mitglied(er)</span>
          <span class="journey-tile-chip chip-status remote">${pct}%</span>
          ${deadlineChip}
          ${badges}
        </div>
        ${membersRow}
