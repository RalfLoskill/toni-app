...
function collectWeeklyPlanItems() {
  ...
  // --- Projekte: zugewiesene/offene Aufgaben mit Verbindlichkeiten ---
  try {
    ...
    tasks.forEach(t => {
      ...
      const overdue = t.due_date && wpIsOverdue(t.due_date);
      const blocked = !!t.blocker;
      let note = '';
      if (blocked) note = 'Blockiert';
      else if (overdue) note = 'überfällig';
      else if (!t.assigned_to) note = 'nicht zugewiesen';
      const assignee = t.assigned_profile ? (t.assigned_profile.first_name || t.assigned_profile.display_name || '') : '';
      items.push({
        source: 'project', title: t.title || '', col,
        meta: `${projTitle[t.project_id] || 'Projekt'}${assignee ? ' · ' + assignee : ''}`,
...
function renderWeeklyPlan() {
  ...
  const colColor = (source) => source === 'journey'
    ? { border:'#185FA5', bg:'#E6F1FB', text:'#0C447C', icon:'ti-book', label:'Lernreise' }
    : { border:'#BA7517', bg:'#FAEEDA', text:'#633806', icon:'ti-folder', label:'Projekt' };
  ...
  const cardHtml = (it) => {
    ...
    else if (it.note === 'nicht zugewiesen') badge = `<span style="font-size:10px;background:var(--color-background-secondary);color:var(--color-text-secondary);padding:1px 6px;border-radius:8px;margin-left:4px">nicht zugewiesen</span>`;
    ...
    return `<div class="k-card${done?' done-c':''}" ${click} style="border-left:3px solid ${c.border};${done?'opacity:.55':''}">
      ...
      <div style="display:flex;align-items:center;gap:5px;font-size:11px;color:${c.text}">
        <i class="ti ${c.icon}" style="font-size:13px"></i><span style="opacity:.85">${escapeHtml(it.meta)}</span>
      </div>
    </div>`;
  };
  ...
  <div style="display:flex;gap:14px;align-items:center;margin-top:8px;padding:0 2px;font-size:11px;color:var(--color-text-tertiary)">
    <span><i class="ti ti-book" style="color:#185FA5"></i> Lernreise</span>
    <span><i class="ti ti-folder" style="color:#BA7517"></i> Projekt</span>
    <span style="margin-left:auto">automatisch aus deinen Lernreisen &amp; Projekten</span>
  </div>`;
}
