...
// Projekte laden – Polling bis TONI_AUTH_PROFILE gesetzt ist
(function waitForAuth() {
  if (window.TONI_AUTH_PROFILE?.id) {
    loadProjects();
    return;
  }
  // applyAuthProfile hooken – wird nach jedem Login aufgerufen
  ...
  window.applyAuthProfile = function(profile) {
    ...
    if (profile?.id) setTimeout(loadProjects, 300);
  };
  // Fallback-Polling für den Fall dass applyAuthProfile schon gelaufen ist
  let tries = 0;
  const poll = setInterval(() => {
    ...
    if (tries > 30) clearInterval(poll); // max 15s
  }, 500);
})();
