...
async function toniGenerateProjectJournal(){
  ...
  try{
    ...
    for(const [k,v] of meta){
      ...
      doc.splitTextToSize(String(v), contentW - 38).forEach((ln, i) => { if(i>0){ y += 5.5; ensureSpace(6); } doc.text(ln, margin + 38, y); });
      y += 7;
    }
    ...
    if(members.length){
      ...
      for(const m of members){
        ...
        if(imgSrc){
          ...
          if(ai){
            doc.addImage(ai.dataUrl, 'JPEG', cx, rowTop, avatarSize, avatarSize);
            drewImg = true;
          }
        }
        if(!drewImg){
    ...
    // ---------- KANBAN-SNAPSHOT (echtes Dashboard als Bild) ----------
    ...
    try{
      ...
      if(board){
        // Volle Breite einfangen, auch den horizontal scrollbaren Teil.
        ...
        setFont(16, 'bold', [12,36,62]); doc.text('Aktueller Stand (Projektboard)', margin, y); y += 9;
        doc.setDrawColor(200,200,200); doc.line(margin, y, pageW - margin, y); y += 6;
        // Bild proportional auf Inhaltsbreite skalieren
        const imgW = contentW;
        const imgH = imgW * (canvas.height / canvas.width);
        const maxH = pageH - margin - y;
        if(imgH > maxH){
          // Zu hoch: an der Höhe ausrichten, zentriert
          const fitH = maxH;
          const fitW = fitH * (canvas.width / canvas.height);
          doc.addImage(imgData, 'JPEG', margin + (contentW - fitW)/2, y, fitW, fitH);
        }else{
