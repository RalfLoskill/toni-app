...
async function loadProjectTasks(projectId) {
  ...
  try {
    ...
    if (!token) return;
    const res = await fetch(
      `${window.SUPABASE_URL}/rest/v1/project_tasks?project_id=eq.${projectId}&order=position.asc,created_at.asc&select=*,assigned_profile:profiles!project_tasks_assigned_to_fkey(id,display_name,first_name,last_name,class_name,role,avatar_url)`,
      { headers: { 'apikey': window.SUPABASE_ANON_KEY, 'Authorization': 'Bearer ' + token } }
    );
    if (!res.ok) throw new Error(await res.text());
    const tasks = await res.json();
...
function toniCanChangeTaskStatus(task) {
  ...
  const isStaff = ['tutor', 'admin', 'superadmin'].includes(role);
  const isAssignee = !!myId && task.assigned_to === myId;
...
async function moveProjectTask(taskId, newStatus) {
  try {
    // V110.1 – Berechtigung pruefen (zugewiesene Person oder Tutor/Admin).
    // Zweite Ebene zusaetzlich zur Button-Sperre in renderTaskCard.
    const known = (window.TONI_PROJECT_TASKS || []).find(t => t.id === taskId);
    if (known && typeof toniCanChangeTaskStatus === 'function' && !toniCanChangeTaskStatus(known)) {
      console.warn('TONI: Statuswechsel nicht erlaubt (keine Zuweisung, keine Tutor-Rolle).');
      return;
    }
    const token = await getToken();
    if (!token) return;
