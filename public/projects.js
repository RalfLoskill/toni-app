...
async function openTaskDetail(taskId) {
  try {
    ...
    document.getElementById('ptask-sub').textContent = p ? (p.title || 'Projekt') : '';
    ...
    const detailsSaveBtn = document.getElementById('ptask-details-save');
    if (isOwner) {
    ...
    const noteSaveBtn = document.getElementById('ptask-note-save');
...
// Status-Buttons im Detailfenster (gleiche Kette/Logik wie im Board)
function renderTaskDetailStatus(task, isOwner) {
  ...
  const fwdBtn = `<button ${fwdEnabled ? `onclick="moveProjectTaskFromDetail('${task.id}','${nextCol}')"` : 'disabled'}
    style="flex:1;padding:9px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);font-size:13px;font-weight:500;
    background:${fwdEnabled?'#185FA5':'var(--color-background-secondary)'};color:${fwdEnabled?'#fff':'var(--color-text-tertiary)'};
    cursor:${fwdEnabled?'pointer':'default'};opacity:${fwdEnabled?'1':'.5'}">${fwdLabel}${nextCol?' →':''}</button>`;

  const resetBtn = `<button ${resetEnabled ? `onclick="moveProjectTaskFromDetail('${task.id}','${prevCol}')"` : 'disabled'}
    title="Zurücksetzen"
    style="padding:9px 12px;border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);font-size:13px;
    background:transparent;color:var(--color-text-primary);cursor:${resetEnabled?'pointer':'default'};opacity:${resetEnabled?'.8':'.35'}">↺</button>`;

  document.getElementById('ptask-status-controls').innerHTML =
    `<span style="font-size:13px;font-weight:500;color:var(--color-text-primary);min-width:80px">${colDE[col]||col}</span>${fwdBtn}${resetBtn}`;
...
// Titel + Beschreibung speichern -> nur Inhaber (RPC erzwingt es)
async function saveTaskDetails() {
  ...
  if (!taskId || !title) { alert('Der Titel darf nicht leer sein.'); return; }
  try {
    ...
    console.warn('TONI Titel/Beschreibung speichern:', e);
    alert(e.message || 'Konnte nicht gespeichert werden.');
  }
}

// Notiz speichern -> nur Inhaber (RPC erzwingt es serverseitig)
async function saveTaskNote() {
  ...
  try {
    ...
    const btn = document.getElementById('ptask-note-save');
    const orig = btn.textContent;
    btn.textContent = '✓ Gespeichert';
    setTimeout(() => { btn.textContent = orig; }, 1500);
  } catch (e) {
    console.warn('TONI Notiz speichern:', e);
