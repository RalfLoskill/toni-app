 ...
 * Build: theme-picker-v3-preview-order
 *
...
`;
  ...
  function init() {
    ...
    refreshTrigger();
    installEditHook();
    // Erneut versuchen, falls journey.js editAdminJourney später nochmals
    // überschreibt (mehrere Wrap-Stufen laden zeitversetzt).
    setTimeout(installEditHook, 600);
    setTimeout(installEditHook, 1500);
    setTimeout(installEditHook, 3000);
  }

  // Robuste Theme-Vorbelegung beim Bearbeiten: hängt sich NACH allen
  // journey.js-Wraps an editAdminJourney und liest das Theme aus der
  // aktiven Reise (TONI_ACTIVE_ADMIN_JOURNEY_ROW). Unabhängig davon, welche
  // editAdminJourney-Version gewinnt – verzögert, um nach Cover-/Builder-
  // Nachbearbeitung zu greifen.
  function themeFromActiveRow() {
    const row = window.TONI_ACTIVE_ADMIN_JOURNEY_ROW;
    if (!row) return null;
    let jj = row.journey_json;
    if (typeof jj === "string") { try { jj = JSON.parse(jj); } catch (e) { jj = {}; } }
    // journey_json.theme ist die maßgebliche Quelle (dort schreibt der Picker);
    // die DB-Spalte row.theme nur als Fallback für Alt-Daten. Reihenfolge ist
    // wichtig: eine veraltete Spalte (z.B. "classic") darf das korrekte Theme
    // aus journey_json (z.B. "space") NICHT überschreiben.
    const t = (jj && jj.theme) || (row.theme) || "";
    return (typeof t === "string" && t.trim()) ? t.trim() : "classic";
  }
  function installEditHook() {
    if (typeof window.editAdminJourney !== "function") {
      // Editor-Funktion evtl. noch nicht da -> später erneut versuchen.
      setTimeout(installEditHook, 300);
      return;
    }
    if (window.editAdminJourney.__toniThemePickerHook) return;
    const orig = window.editAdminJourney;
    window.editAdminJourney = function () {
      const r = orig.apply(this, arguments);
      // Nach allen synchronen + verzögerten Nachbearbeitungen das Theme setzen.
      setTimeout(function () {
        const t = themeFromActiveRow();
        if (t) select(t);
      }, 200);
      return r;
    };
    window.editAdminJourney.__toniThemePickerHook = true;
  }
  ...
  console.info("[TONI-Theme-Picker] geladen (theme-picker-v3-preview-order).");
})();
