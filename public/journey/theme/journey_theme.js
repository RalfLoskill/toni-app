...
(function () {
  ...
  window.TONI_THEME_BUILD = "theme-v3-fullscreen-cleanup";
  ...
  /** Liefert die Liste registrierter Themes (für die Editor-Galerie),
   *  inklusive eines Vorschau-SVG (falls das Theme renderPreview anbietet). */
  function listThemes() {
    return Object.keys(THEMES).map(function (id) {
      ...
      return {
        ...
        preview: previewFor(t.id)
      };
    });
  }

  /** Liefert das Vorschau-SVG eines Themes (oder ein neutrales Fallback-SVG). */
  function previewFor(id) {
    const t = THEMES[id];
    if (t && typeof t.renderPreview === "function") {
      try {
        const svg = t.renderPreview();
        if (typeof svg === "string" && svg.length > 0) return svg;
      } catch (e) {
        console.warn("[TONI-Theme] renderPreview von '" + id + "' fehlgeschlagen.", e);
      }
    }
    // Neutrales Fallback (graue Fläche mit Label-Andeutung).
    return '<svg viewBox="0 0 160 100" xmlns="http://www.w3.org/2000/svg">' +
           '<rect width="160" height="100" rx="10" fill="#eef2f7"/>' +
           '<rect x="20" y="34" width="120" height="8" rx="4" fill="#c7d2e0"/>' +
           '<rect x="20" y="52" width="80" height="8" rx="4" fill="#dbe3ec"/></svg>';
  }
  ...
  function applyModalThemeClass(themeId) {
    ...
    // FREMDE Vollbild-Klassen am <body> entfernen. Jedes Theme verwaltet nur
    // seine eigene *-fullscreen-Klasse; beim Wechsel bleibt sonst die alte
    // hängen und die vorherige Anzeige "klebt". Daher hier zentral bereinigen:
    // alle toni-*-fullscreen entfernen AUSSER der des aktiven Themes. Das
    // aktive Theme setzt seine Klasse anschließend in renderStations() selbst.
    if (document.body && document.body.classList) {
      const keep = "toni-" + themeId + "-fullscreen";
      const bodyRemove = [];
      document.body.classList.forEach(function (c) {
        if (/^toni-.+-fullscreen$/.test(c) && c !== keep) bodyRemove.push(c);
      });
      bodyRemove.forEach(function (c) { document.body.classList.remove(c); });
    }
  }
  ...
  registerTheme({
    ...
    // Mini-Vorschau: ruhige Liste, wie sie classic darstellt.
    renderPreview: function () {
      return '<svg viewBox="0 0 160 100" xmlns="http://www.w3.org/2000/svg">' +
        '<rect width="160" height="100" rx="10" fill="#f7f6f2"/>' +
        '<circle cx="24" cy="26" r="7" fill="#639922"/>' +
        '<rect x="38" y="22" width="92" height="8" rx="4" fill="#cdd6e0"/>' +
        '<circle cx="24" cy="50" r="7" fill="#185FA5"/>' +
        '<rect x="38" y="46" width="78" height="8" rx="4" fill="#dbe3ec"/>' +
        '<circle cx="24" cy="74" r="7" fill="#c7d0db"/>' +
        '<rect x="38" y="70" width="64" height="8" rx="4" fill="#e4e9f0"/></svg>';
    }
  });
