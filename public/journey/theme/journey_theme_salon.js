...
(function () {
  ...
  function injectStyles() {
    ...
    const css =
      ...
      ".toni-salon__mirror{position:absolute;top:64px;left:50%;transform:translateX(-50%);z-index:1;" +
        ...
        // klassischer Goldrahmen mit Verlauf + Profil
        "border:14px solid;border-image:linear-gradient(145deg,#FFF1B8,#E8B84B 35%,#B8860B 70%,#FFE9A8) 1;" +
        "box-shadow:inset 0 0 0 3px #C9961F,inset 0 8px 30px rgba(0,0,0,.06)," +
        "0 0 0 2px #9C7515,0 18px 40px rgba(0,0,0,.18);overflow:visible;}" +
      ...
      // Lichterkette: einzelne weiße Glühlampen rund um den Rahmen
      ".toni-salon__bulbs{position:absolute;inset:-7px;z-index:3;pointer-events:none;}" +
      ".toni-salon__bulbs i{position:absolute;width:12px;height:12px;border-radius:50%;transform:translate(-50%,-50%);" +
        "background:radial-gradient(circle at 38% 32%,#FFFFFF,#FFFDF5 55%,#F3ECD8);" +
        "box-shadow:0 0 8px 2px rgba(255,255,255,.95),0 0 14px 4px rgba(255,250,235,.7);" +
      ...
      ".toni-salon__it{position:absolute;bottom:14px;}" +
      ...
      // Shampoo-Pumpflasche (mit Pumpkopf)
      ".toni-salon__it-shampoo{width:14px;height:34px;border-radius:4px 4px 3px 3px;background:linear-gradient(180deg,#FFF,var(--ic,#FF8FAB));" +
        "box-shadow:0 -7px 0 -3px #9a8 inset;}" +
      ".toni-salon__it-shampoo::after{content:'';position:absolute;top:-9px;left:50%;width:9px;height:9px;" +
        "transform:translateX(-40%);border-radius:2px;background:#bbb;}" +
      // Föhn (klein, im Regal)
      ".toni-salon__it-dryer{width:26px;height:18px;border-radius:9px 4px 4px 9px;background:var(--ic,#FF7B54);}" +
      ".toni-salon__it-dryer::after{content:'';position:absolute;bottom:-9px;left:6px;width:7px;height:12px;" +
        "border-radius:3px;background:var(--ic,#FF7B54);}" +
      // Rasierer
      ".toni-salon__it-razor{width:8px;height:30px;border-radius:3px;background:#cfd3d6;}" +
      ".toni-salon__it-razor::after{content:'';position:absolute;top:-4px;left:-2px;width:12px;height:6px;" +
        "border-radius:2px;background:#9aa0a6;}" +
      // Ordner (stehend, Rücken sichtbar)
      ".toni-salon__it-folder{width:9px;height:38px;border-radius:2px;background:var(--ic,#5BB0E0);" +
        "box-shadow:2px 0 0 #fff,4px 0 0 var(--ic2,#FF9F68),11px 0 0 #fff,13px 0 0 var(--ic3,#FFD166);}" +
      // gerolltes Handtuch
      ".toni-salon__it-towel{width:24px;height:14px;border-radius:7px;background:linear-gradient(90deg,#fff,#FBEFE2);" +
        "box-shadow:inset -4px 0 0 rgba(0,0,0,.06);}" +
      ...
      "@keyframes toniSalonBulb{0%,100%{opacity:.7;box-shadow:0 0 5px 1px rgba(255,255,255,.7);}50%{opacity:1;box-shadow:0 0 12px 4px rgba(255,255,255,1);}}" +
      ...
      // ===== Mobile: Stationen nicht-überlappend stapeln (kein absolutes %-Layout) =====
      // ===== Schmale Viewports: Stationen nicht-überlappend stapeln (kein absolutes %-Layout) =====
      "@media (max-width:820px){" +
        // Bühne wird normaler Fluss-Block statt Aspect-Ratio-Fläche
        ".toni-salon__stage{padding-bottom:0 !important;height:auto !important;" +
          "display:flex;flex-wrap:wrap;gap:14px 10px;justify-content:center;align-content:flex-start;" +
          "padding:14px 10px 6px !important;}" +
        // Stationen: nicht mehr absolut, sondern im Fluss; feste, kleinere Breite
        ".toni-salon__seat{position:static !important;left:auto !important;top:auto !important;" +
          "transform:none !important;width:46% !important;max-width:200px;}" +
        ".toni-salon__seat:hover{transform:translateY(-2px) !important;}" +
        // Regale schmaler, damit sie auf Mobil nicht ins Bild ragen
        ".toni-salon__panel{width:13% !important;}" +
        // Spiegel sicher zentriert in den Fluss setzen (nicht mehr absolut rechts)
        ".toni-salon__mirror{position:relative !important;top:auto !important;left:auto !important;right:auto !important;" +
          "transform:none !important;margin:6px auto 14px !important;display:block;width:min(58%,210px) !important;}" +
        // Überlappenden Kopf-Text + Markenschild ausblenden (nur Buttons bleiben)
        ".toni-salon__brand{display:none !important;}" +
        ".toni-salon__head{display:none !important;}" +
        // Werkzeug-Matte auf Mobil etwas schmaler
        ".toni-salon__mat{width:min(82%,420px) !important;}" +
        // Bodennahe Deko ausblenden – sie kollidiert sonst mit den gestapelten Stationen.
        ".toni-salon__counter,.toni-salon__chair,.toni-salon__mat,.toni-salon__deco{display:none !important;}" +
      "}" +
      "@media (max-width:380px){" +
        ".toni-salon__seat{width:100% !important;max-width:260px;}" +
      "}";
  ...
  /* ---- Seitliche Produktregale (links + rechts), voll gefüllt ---- */
  function panelsHTML() {
    function shelf(top, items) {
      ...
      items.forEach(function (it, i) {
        ...
        its += '<span class="toni-salon__it toni-salon__it-' + it.k + '" ' +
          'style="left:' + left.toFixed(0) + '%;transform:translateX(-50%);' +
          '--ic:' + (it.c || PINK) + (it.c2 ? ';--ic2:' + it.c2 : '') + (it.c3 ? ';--ic3:' + it.c3 : '') + '"></span>';
      });
      return '<div class="toni-salon__shelf" style="top:' + top + '%">' + its + '</div>';
    }
    const left =
      '<div class="toni-salon__panel l">' +
        shelf(15, [{k:"shampoo",c:PINK},{k:"shampoo",c:CORAL},{k:"tall",c:PEACH},{k:"tube",c:"#5BB0E0"},{k:"bottle",c:PINK}]) +
        shelf(37, [{k:"dryer",c:CORAL},{k:"jar",c:GOLD},{k:"bottle",c:PINK},{k:"razor"}]) +
        shelf(59, [{k:"folder",c:"#5BB0E0",c2:CORAL,c3:GOLD},{k:"box",c:PEACH},{k:"jar",c:PINK}]) +
        shelf(81, [{k:"towel"},{k:"candle"},{k:"plant"},{k:"shampoo",c:GOLD}]) +
      '</div>';
    const right =
      '<div class="toni-salon__panel r">' +
        shelf(15, [{k:"shampoo",c:CORAL},{k:"tall",c:PINK},{k:"shampoo",c:PEACH},{k:"tube",c:"#5BB0E0"},{k:"bottle",c:GOLD}]) +
        shelf(37, [{k:"razor"},{k:"bottle",c:PINK},{k:"dryer",c:PINK},{k:"jar",c:GOLD}]) +
        shelf(59, [{k:"box",c:PEACH},{k:"folder",c:GOLD,c2:"#5BB0E0",c3:CORAL},{k:"bottle",c:CORAL}]) +
        shelf(81, [{k:"plant"},{k:"shampoo",c:PINK},{k:"towel"},{k:"candle"}]) +
      '</div>';
    return left + right;
  }
  ...
  function renderStations(journey) {
    ...
    // Lichterkette: weiße Glühlampen rund um den Rahmen (oben gerundet, Seiten, unten)
    let bulbs = "";
    let bi = 0;
    function bulb(xp, yp) {
      bulbs += '<i style="left:' + xp.toFixed(1) + '%;top:' + yp.toFixed(1) + '%;animation-delay:' + (bi * 0.13).toFixed(2) + 's"></i>';
      bi++;
    }
    // Oberer Bogen (Halbkreis), dichter
    for (let i = 0; i <= 14; i++) {
      const ang = (i / 14) * Math.PI;
      bulb(50 + Math.cos(Math.PI - ang) * 48, 20 - Math.sin(ang) * 19);
    }
    // Linke + rechte Seite (vertikal hinunter)
    for (let i = 1; i <= 8; i++) {
      const y = 20 + (i / 8) * 78;
      bulb(2, y); bulb(98, y);
    }
    ...
    return '<div class="toni-salon">' +
      ...
      '<div class="toni-salon__stage" style="position:relative;width:100%;padding-bottom:' + aspect + '%;">' +
        seatsHTML +
      '</div>' +
  ...
  console.info("[TONI-Theme:salon] Friseursalon-Theme registriert (theme-salon-v5-mobile2).");
})();
