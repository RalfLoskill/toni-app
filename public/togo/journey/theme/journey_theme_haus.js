...
(function () {
  ...
  window.toniHausToggleRoom = function (index) {
    ...
    if (already) { toniHausCloseDock(); return; }
    room.classList.add("open");
    // Aufgaben des Raums in die feste Bottom-Leiste spiegeln (Punkt 1)
    const dock = document.getElementById("toni-haus-dock");
    const dockBody = document.getElementById("toni-haus-dock-body");
    const dockTitle = document.getElementById("toni-haus-dock-title");
    const src = room.querySelector(".toni-haus__switches");
    if (dock && dockBody) {
      dockBody.innerHTML = src ? src.innerHTML :
        '<div class="toni-haus__dock-empty">Dieser Raum hat noch keine Aufgaben.</div>';
      if (dockTitle) {
        const label = room.getAttribute("data-room-title") || ("Raum " + (index + 1));
  ...
  window.toniHausCloseDock = function () {
    const dock = document.getElementById("toni-haus-dock");
    if (dock) dock.classList.remove("open");
    const root = document.querySelector(".toni-haus");
    if (root) root.querySelectorAll(".toni-haus__room.open").forEach(function (r) { r.classList.remove("open"); });
  };
...
.toni-haus__moon{position:absolute;top:34px;left:6%;width:192px;height:192px;border-radius:50%;z-index:0;
  background:
    radial-gradient(circle at 32% 30%,rgba(255,255,255,.5),transparent 22%),
    radial-gradient(circle at 70% 64%,rgba(120,110,90,.45),transparent 26%),
    radial-gradient(circle at 50% 50%,rgba(150,140,115,.35),transparent 30%),
    radial-gradient(circle at 40% 38%,#FBF6E9 0%,#EFE6C9 45%,#D8CBA2 72%,#BCAE83 100%);
  box-shadow:inset -22px -18px 44px rgba(60,52,34,.55),
    inset 14px 12px 30px rgba(255,252,240,.5),
    0 0 70px rgba(251,246,233,.4),0 0 120px rgba(251,246,233,.18);}
/* einzelne Krater */
.toni-haus__moon::before{content:'';position:absolute;border-radius:50%;
  top:30%;left:54%;width:26px;height:26px;
  background:radial-gradient(circle at 40% 40%,rgba(120,110,88,.5),rgba(170,158,128,.15));
  box-shadow:38px 26px 0 -6px rgba(120,110,88,.32),-30px 40px 0 -10px rgba(120,110,88,.28),
    10px 58px 0 -4px rgba(120,110,88,.3),-44px -6px 0 -12px rgba(120,110,88,.22);}

/* Punkt 1: Aufgaben-Dock am unteren Bildschirmrand */
.toni-haus__switches{display:none !important;}
.toni-haus__dock{position:fixed;left:0;right:0;bottom:0;z-index:4600;
  background:linear-gradient(180deg,rgba(28,20,32,.96),rgba(18,12,22,.98));
  ...
  padding:12px 18px calc(14px + env(safe-area-inset-bottom));backdrop-filter:blur(6px);}
.toni-haus__dock.open{transform:translateY(0);}
.toni-haus__dock-head{display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;}
.toni-haus__dock-title{color:var(--light);font-weight:700;font-size:15px;letter-spacing:.3px;}
.toni-haus__dock-close{flex:0 0 auto;background:#171019;color:var(--accent);
  border:1.5px solid var(--accent);border-radius:10px;padding:7px 14px;font-size:13px;cursor:pointer;
  font-weight:600;transition:background .12s;}
.toni-haus__dock-close:hover{background:#241a10;}
.toni-haus__dock-body{display:flex;flex-wrap:wrap;gap:12px;justify-content:center;
  max-height:38vh;overflow-y:auto;overscroll-behavior:contain;padding-bottom:4px;}
.toni-haus__dock-body .toni-haus__switch{display:flex;width:88px;}
.toni-haus__dock-empty{color:#C9BDD6;padding:14px;text-align:center;width:100%;}
/* Punkt 3: Home-Button im Kopf klar erkennbar (eigener Glas-Rahmen) */
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .toni-haus__keybtn{
  background:rgba(23,16,25,.85) !important;border:1.5px solid var(--accent) !important;
  box-shadow:0 0 16px rgba(255,184,56,.45) !important;}
...
/* Grid einspaltig: Stationen füllen die volle Breite (sonst bleibt die schmale Box links) */
.toni-theme-active-haus .lr-top-split{
  grid-template-columns:1fr !important;grid-template-areas:"stations" "right" !important;}
.toni-theme-active-haus .lr-top-split .lr-stations{background:transparent !important;padding:0 !important;}
/* Punkt 2: Karten-Kopf "Stationen der Lernreise"/"Aktualisieren" entfernen */
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .card-header{display:none !important;}
/* Punkt 2: auch den Reise-Titel im Kopf ausblenden */
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .lr-modal-title{display:none !important;}
/* Punkt 3: Header transparent, Buttons (Journal/Schließen) als Glas wie Weltall */
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .lr-modal-header{
...
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .lr-modal-actions{
...
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .lr-modal-actions button{
  flex:0 0 auto !important;position:static !important;margin:0 !important;
  background:rgba(23,16,25,.72) !important;color:${ACCENT} !important;
  border:1px solid ${ACCENT} !important;backdrop-filter:blur(4px);}
body.toni-haus-fullscreen #lr-modal.toni-theme-active-haus .lr-modal-actions button[onclick*="startNextLearningTask"]{display:none !important;}
...
/* Punkt 5: Schließen-Button (und andere Buttons) im Aufgaben-Detail mit schwarzer Schrift */
body.toni-haus-fullscreen #lr-task-modal .lr-modal-header button,
body.toni-haus-fullscreen #lr-task-modal .lr-modal-header button *,
body.toni-haus-fullscreen #lr-task-modal .lr-close-btn,
body.toni-haus-fullscreen #lr-task-modal .lr-close-btn *{color:#111 !important;}
body.toni-haus-fullscreen #lr-task-modal .lr-modal-header button,
body.toni-haus-fullscreen #lr-task-modal .lr-close-btn{
  background:#F3EEE6 !important;border:1px solid ${ACCENT} !important;}
...
`;
  ...
  function renderStations(journey) {
    ...
    return '' +
      '<div class="toni-haus">' +
        ...
        // Punkt 1: feste Aufgaben-Leiste am unteren Bildschirmrand
        '<div class="toni-haus__dock" id="toni-haus-dock">' +
          '<div class="toni-haus__dock-head">' +
            '<span class="toni-haus__dock-title" id="toni-haus-dock-title">Aufgaben</span>' +
            '<button type="button" class="toni-haus__dock-close" ' +
              'onclick="toniHausCloseDock()" aria-label="Aufgaben schließen">Schließen ✕</button>' +
          '</div>' +
          '<div class="toni-haus__dock-body" id="toni-haus-dock-body"></div>' +
        '</div>' +
      '</div>';
  }
  ...
  function renderRoom(step, index, state) {
    ...
    return '' +
      '<div class="toni-haus__room ' + state + ' toni-tl-nav" data-step-index="' + index + '" ' +
        'data-room-title="' + esc(step.title || ("Station " + (index + 1))) + '" ' +
  ...
  console.info("[TONI-Theme:haus] Wissenshaus-Theme registriert (theme-haus-v4-dock-moon).");
})();
