...
(function () {
  ...
  function miniPoints(insts, detArr, stArr) {
    var det = {}, st = {};
    detArr.forEach(function (d) { det[d.institution_id] = d; });
    stArr.forEach(function (s) { st[s.institution_id] = s; });
  ...
  async function renderMiniMap() {
    if (!$("minimap")) return;
    var r = await Promise.all([
      sb.from("institutions").select("id,name,city"),
    ...
    var pts = miniPoints(pick(r[0]), pick(r[1]), pick(r[2]));
    ...
    pts.forEach(function (p) {
      L.circleMarker([Number(p.lat), Number(p.lng)], {
        radius: 6, color: "#fff", weight: 2, fillColor: B.statusColor(p.status), fillOpacity: .9
