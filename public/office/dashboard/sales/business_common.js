...
window.BC = (function () {
  ...
  var SOURCE_LABEL = {
    public_website: "Öffentliche Website", manual_entry: "Selbst erfasst",
    referral: "Empfehlung", event: "Veranstaltung", inbound: "Selbst gemeldet"
  };
  function sourceLabel(k) { return SOURCE_LABEL[k] || "–"; }
  function sourceOptions(selected) {
    var keys = ["", "public_website", "manual_entry", "referral", "event", "inbound"];
    return keys.map(function (k) {
      return '<option value="' + k + '"' + (k === (selected || "") ? " selected" : "") + '>' +
        (k === "" ? "–" : esc(SOURCE_LABEL[k])) + '</option>';
    }).join("");
  }
  // Einwilligungsstatus eines Kontakts + Ampelfarbe
  var CONSENT = [
    ["unknown",   "Unbekannt",           "#9ca3af"],  // öffentlich recherchiert
    ["soft_optin","Berechtigtes Interesse","#f59e0b"],
    ["opt_in",    "Einwilligung liegt vor","#22c55e"],
    ["opt_out",   "Widerspruch",         "#dc2626"]
  ];
  function consentLabel(k) { for (var i = 0; i < CONSENT.length; i++) if (CONSENT[i][0] === k) return CONSENT[i][1]; return k; }
  function consentColor(k) { for (var i = 0; i < CONSENT.length; i++) if (CONSENT[i][0] === k) return CONSENT[i][2]; return "#9ca3af"; }
  function consentOptions(selected) {
    return CONSENT.map(function (c) {
      return '<option value="' + c[0] + '"' + (c[0] === (selected || "unknown") ? " selected" : "") + '>' + esc(c[1]) + '</option>';
    }).join("");
  }
