...
(function () {
  ...
  async function load(role) {
    ...
    var res = await Promise.all([
      sb.from("v_crm_institution_stage").select("*").order("name"),
      sb.from("v_crm_institution_contactability").select("*")
    ]);
    ALL = data(res[0]);
    // Kontaktierbarkeit je Schule anfügen
    var cby = {};
    data(res[1]).forEach(function (c) { cby[c.institution_id] = c; });
    ALL.forEach(function (r) {
      var c = cby[r.institution_id];
      r.contactability = c ? c.contactability : "no_contacts";
      r.contactable_count = c ? c.contactable_count : 0;
    });
    render();
  }
  ...
  function render() {
    ...
    var cf = ($("contact-filter") ? $("contact-filter").value : "");
    var rows = ALL.filter(function (r) {
      if (sf && r.lifecycle_stage !== sf) return false;
      if (cf && (r.contactability || "no_contacts") !== cf) return false;
    ...
    $("rows").innerHTML = rows.map(function (r) {
      ...
      return '<tr class="clickable" data-id="' + r.institution_id + '">' +
        '<td><b>' + B.esc(r.name) + '</b>' + (r.is_partner ? ' <span class="pill low">Partner</span>' : "") +
          contactBadge(r.contactability) + '</td>' +
  ...
  // Dezente Markierung: nur Sperre und "keiner erreichbar" werden gezeigt.
  // "contactable" und "no_contacts" bleiben ohne Badge (kein Rauschen).
  function contactBadge(c) {
    if (c === "blocked") return ' <span class="pill high" title="Kontaktsperre für die ganze Schule">Gesperrt</span>';
    if (c === "no_contactable") return ' <span class="pill low" title="Kontakte vorhanden, aber keiner kontaktierbar">nicht erreichbar</span>';
    return "";
  }
  ...
  function wireUi() {
    ...
    var cf = $("contact-filter"); if (cf) cf.addEventListener("change", render);
    ...
    $("f-country").addEventListener("change", syncStateEnabled);
    $("new-modal").addEventListener("click", function (e) { if (e.target === $("new-modal")) closeModal(); });
  }

  function openModal() {
    $("new-msg").textContent = "";
    $("f-country").innerHTML = B.countryOptions("Deutschland");
    $("f-state").innerHTML = B.stateOptions("");
    syncStateEnabled();
    $("new-modal").style.display = "flex"; $("f-name").focus();
  }
  // Bundesland nur aktiv, wenn Land = Deutschland
  function syncStateEnabled() {
    var de = $("f-country").value === "Deutschland";
    $("f-state").disabled = !de;
    $("f-state").style.opacity = de ? "" : ".5";
    if (!de) $("f-state").value = "";
  }
  function closeModal() {
    $("new-modal").style.display = "none";
    ["f-name","f-city","f-postal","f-street","f-website"].forEach(function (id) { $(id).value = ""; });
    $("f-type").value = "";
  }
  ...
  async function save() {
    ...
    try {
      ...
      var det = await sb.from("crm_institution_details").upsert({
        ...
        country: $("f-country").value || "Deutschland",
        state: ($("f-country").value === "Deutschland" ? (val("f-state")) : null),
  ...
  window.TONI_BC_LIST = { setData: function (a) { ALL = a; }, render: render, wire: wireUi, openModal: openModal };
