...
(function () {
  ...
  async function runFind() {
    ...
    $("run-title").textContent = "Arbeitsverlauf · Lead-Suche";
    $("run-chat").innerHTML = "";
    $("run-result").style.display = "none"; $("run-result").innerHTML = "";
    pushChat("agent", ACTIVE.name + " sucht nach passenden Schulen …");
    pushChat("system", "…", true);

    // bestehende Institutionen laden (Dubletten-Ausschluss: name|plz)
    var existing = [];
    try {
      var ex = await sb.from("v_crm_institution_stage").select("name,postal_code");
      existing = data(ex).map(function (i) { return (String(i.name || "") + "|" + String(i.postal_code || "")).toLowerCase().trim(); });
    } catch (e) {}
    ...
    var payload = {
      ...
      criteria: c.criteria, freeText: c.freeText, existing: existing, maxLeads: 10
    };

    try {
      var resp = await fetch(API_ENDPOINT, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      removeTyping();
      if (!resp.ok) {
        var errTxt = ""; try { errTxt = (await resp.json()).error || ""; } catch (e) {}
        pushChat("system", "Die Suche ist fehlgeschlagen. " + errTxt);
        if (runId) await sb.from("crm_agent_runs").update({ status: "error", error_text: errTxt }).eq("id", runId);
        return;
      }
      var out = await resp.json();
      (out.chat || []).forEach(function (cc) { pushChat("agent", cc.content); });
      if (runId) {
        var chatRows = (out.chat || []).map(function (cc, i) { return { run_id: runId, seq: i + 1, role: cc.role || "agent", content: cc.content }; });
      ...
      FOUND = (out.result && out.result.leads) || [];
      renderFoundLeads(out.usage || {});
      await loadCosts();
    } catch (e) {
      removeTyping();
      pushChat("system", "Netzwerkfehler bei der Suche.");
  ...
  function renderFoundLeads(usage) {
    if (!FOUND.length) { $("run-result").style.display = "none"; return; }
    var cost = usage && usage.cost_eur != null ? ' · Kosten: ' + euro2(usage.cost_eur) : "";
    var searches = usage && usage.searches ? ' · ' + usage.searches + ' Websuchen' : "";
    var rows = FOUND.map(function (l, i) {
      return '<div class="found-item">' +
        '<label class="found-check"><input type="checkbox" data-i="' + i + '" checked></label>' +
        '<div class="found-main">' +
          '<div class="found-name">' + B.esc(l.name) + '</div>' +
          '<div class="found-sub">' + B.esc([l.school_type, l.postal_code, l.city].filter(Boolean).join(" · ")) + '</div>' +
          (l.why ? '<div class="found-why">' + B.esc(l.why) + '</div>' : "") +
          '<div class="found-links">' +
            (l.website ? '<a href="' + B.esc(l.website) + '" target="_blank" rel="noopener">Website ↗</a>' : "") +
            '<a href="' + B.esc(l.source_url) + '" target="_blank" rel="noopener">Beleg ↗</a>' +
          '</div>' +
        '</div></div>';
    }).join("");
    $("run-result").style.display = "";
    $("run-result").innerHTML =
      '<div class="found-head"><b>' + FOUND.length + ' Vorschläge</b><span class="found-meta">' + searches + cost + '</span></div>' +
      '<p class="found-hint">Häkchen setzen bei den Schulen, die du übernehmen willst. Prüfe die Belege.</p>' +
      rows +
      '<div class="res-actions"><button class="btn ghost sm" id="found-none">Nichts übernehmen</button>' +
        '<button class="btn sm" id="found-apply">Ausgewählte übernehmen</button></div>';
    $("found-apply").addEventListener("click", applyFound);
    $("found-none").addEventListener("click", function () {
      pushChat("system", "Keine Vorschläge übernommen.");
      $("run-result").style.display = "none"; FOUND = [];
    });
  }

  // Ausgewählte Vorschläge als Institutionen anlegen
  async function applyFound() {
    var checks = $("run-result").querySelectorAll('.found-check input[type=checkbox]');
    var chosen = [];
    Array.prototype.forEach.call(checks, function (cb) { if (cb.checked) chosen.push(FOUND[parseInt(cb.getAttribute("data-i"), 10)]); });
    if (!chosen.length) { pushChat("system", "Keine Schule ausgewählt."); return; }

    $("found-apply").disabled = true;
    var created = 0;
    for (var i = 0; i < chosen.length; i++) {
      var l = chosen[i];
      try {
        var ins = await sb.from("institutions").insert({
          name: l.name, city: l.city || null, postal_code: l.postal_code || null, street: l.street || null
        }).select("id").single();
        if (ins.error) throw ins.error;
        var id = ins.data.id;
        // Herkunft dokumentieren (DSGVO): public_website + Beleg-URL
        var det = await sb.from("crm_institution_details").upsert({
          ...
          source_urls: [l.source_url].concat(l.website ? [l.website] : []),
          first_collected_at: new Date().toISOString()
        }, { onConflict: "institution_id" });
        if (det.error) throw det.error;
        // Website an die Institution, falls Spalte vorhanden (optional, ignoriert Fehler)
        if (l.website) { try { await sb.from("institutions").update({ website: l.website }).eq("id", id); } catch (e) {} }
        created++;
      } catch (e) { /* einzelne Fehler überspringen */ }
    }
    pushChat("agent", "Erledigt — ich habe " + created + " von " + chosen.length + " Schulen als neue Leads angelegt.");
    var ra = $("run-result").querySelector(".res-actions");
    if (ra) ra.innerHTML = '<span class="res-applied">✓ ' + created + ' übernommen</span>';
    FOUND = [];
  }
