...
(function () {
  ...
  async function acceptSelected() {
    ...
    var created = 0, contactsCreated = 0, errors = [];
    ...
    for (var i = 0; i < chosen.length; i++) {
      ...
      try {
        ...
        var det = await sb.from("crm_institution_details").upsert({
          ...
          state: r.state || null, website: r.website || null,
        ...
        if (contacts.length) {
          ...
          var cr = await sb.from("crm_contacts").insert(contactRows);
          if (cr.error) { console.warn("Kontakte:", cr.error.message); }
          else contactsCreated += contactRows.length;
        }

        // Query-Eintrag auf accepted setzen + verknüpfen
        var upd = await sb.from("crm_query").update({
          status: "accepted", institution_id: id, decided_at: new Date().toISOString()
        }).eq("id", r.id);
        if (upd.error) throw upd.error;
        created++;
      } catch (e) {
        var msg = (e && e.message) ? e.message : String(e);
        console.error("Übernahme fehlgeschlagen für", r.name, e);
        errors.push(r.name + ": " + msg);
      }
    }

    if (created) {
      qmsg(created + " als Leads übernommen" + (contactsCreated ? (", " + contactsCreated + " Kontakte angelegt.") : ".")
        + (errors.length ? (" " + errors.length + " fehlgeschlagen.") : ""), errors.length ? "err" : "ok");
    } else {
      qmsg("Keine Übernahme möglich. " + (errors[0] || "Unbekannter Fehler."), "err");
    }
  ...
  async function discardSelected() {
    ...
    for (var i = 0; i < chosen.length; i++) {
      try {
        var res = await sb.from("crm_query").update({
          status: "discarded", decided_at: new Date().toISOString()
        }).eq("id", chosen[i].id);
        if (res.error) throw res.error;
        done++;
      } catch (e) {
        console.error("Verwerfen fehlgeschlagen für", chosen[i].name, e);
        errs.push((e && e.message) ? e.message : String(e));
      }
    }
    if (done) qmsg(done + " verworfen." + (errs.length ? (" " + errs.length + " fehlgeschlagen.") : ""), errs.length ? "err" : "ok");
    else qmsg("Verwerfen fehlgeschlagen. " + (errs[0] || ""), "err");
    await refresh();
  }
