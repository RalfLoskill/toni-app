...
(function () {
  ...
  var ROLE = null;
  var canWrite = false;
  var GOAL_KEY = "toni_bc_month_goal";
  ...
  function getGoal() {
    var g = null;
    try { g = parseFloat(window.localStorage.getItem(GOAL_KEY)); } catch (e) {}
    return (g && g > 0) ? g : 25000;
  }
  ...
  function renderSuccess() {
    ...
    if (ed) ed.addEventListener("click", function () {
      var cur = getGoal();
      var v = window.prompt("Monatsziel in € (Netto-Umsatz gewonnener Abschlüsse):", cur);
      if (v == null) return;
      var n = parseFloat(String(v).replace(/[^\d.,]/g, "").replace(",", "."));
      if (n > 0) { try { window.localStorage.setItem(GOAL_KEY, n); } catch (e) {} renderSuccess(); }
    });
  }
  ...
  // Für Tests
  window.TONI_BC_PIPE = {
    seed: function (opps, names) { OPPS = {}; opps.forEach(function (o) { OPPS[o.id] = o; }); INSTNAME = names || {}; canWrite = true; },
